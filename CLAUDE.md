...
its own task on `bugFishingFixPipelineId`, else `pl_bugfix_tested`. Trap: marking is accepted MID-hunt,
so the state survives `resetStepForRerun` (the loop re-arms one step) and every reduction is over the
ACCUMULATED catch. Doc: [`bug-fishing-expedition.md`](./docs/initiatives/bug-fishing-expedition.md).
