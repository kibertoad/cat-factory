...
| [`backend/internal/example-custom-agent`](./backend/internal/example-custom-agent) | `@cat-factory/example-custom-agent` | Worked example of a company-authored agent package registered purely via the public app-owned registries (`AgentKindRegistry` + `PipelineRegistry`, by reference) — a repo-writing agent that ships with zero harness changes.                                                          |
