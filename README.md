...
| **PR verification report**        | The ENGINE (not the agent) maintains a structured verification report on each run's PR: the `ci` gate's aggregated check runs + fixer attempts, the tester's structured report, the ephemeral-environment lifecycle, the merger's scored assessment, run metadata (linked tracker issues, pipeline, per-step agent kind + model) and a deep link into the run's observability panel. Human-readable markdown plus a fenced, schema-validated JSON block, updated **idempotently in place** on re-runs. Provider-neutral — a GitLab MR gets it too. See [`docs/initiatives/pr-verification-report.md`](./docs/initiatives/pr-verification-report.md). |
