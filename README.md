...
| **Acceptance criteria**           | Each service accumulates a durable behaviour contract — given/when/outcome statements of what it must do — instead of losing that knowledge with each task's requirements review. Criteria are proposed automatically by an extraction pass whenever a requirements review settles, confirmed or retired by a human, then given to every agent that touches the service; the tester returns a per-criterion verdict and the PR verification report renders the criterion → evidence table. See [`docs/initiatives/acceptance-criteria-store.md`](./docs/initiatives/acceptance-criteria-store.md).                                                                                                                                                                                                                                 |
...
| **PR verification report**        | The ENGINE (not the agent) maintains a structured verification report on each run's PR: the `ci` gate's aggregated check runs + fixer attempts, the tester's structured report, the ephemeral-environment lifecycle, the merger's scored assessment, run metadata (linked tracker issues, pipeline, per-step agent kind + model) and a deep link into the run's observability panel. Human-readable markdown plus a fenced, schema-validated JSON block, updated **idempotently in place** on re-runs. Credentials are scrubbed and the host's auto-link/issue-closing triggers are neutralised before anything is written; per-workspace opt-out via `publishPrVerificationReport`. Provider-neutral — a GitLab MR gets it too. See [`docs/initiatives/pr-verification-report.md`](./docs/initiatives/pr-verification-report.md). |
...
| [`backend/internal/example-custom-agent`](./backend/internal/example-custom-agent) | `@cat-factory/example-custom-agent` | Worked example of a company-authored extension package registered purely via the public app-owned registries (by reference) — a repo-writing agent that ships with zero harness changes, plus a custom test-infrastructure provider with a multi-file `detect()` autodetection hook (`registerExampleStackDeployProvider`). |
