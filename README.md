...
| [`deploy/preview`](./deploy/preview)   | —                              | Per-PR **test environments for cat-factory itself** (no package): a single-origin docker-compose stack for local deployments, and a per-PR Cloudflare Worker + D1 + Pages preview for the Worker facade. See [its README](./deploy/preview/README.md) and [`docs/dogfooding.md`](./docs/dogfooding.md).                                                                             |
