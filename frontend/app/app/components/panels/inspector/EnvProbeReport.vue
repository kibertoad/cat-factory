...
const verdictClass = computed(() => VERDICT_CLASS[props.report.verdict] ?? 'text-toned')
...
<template>
  <div class="space-y-2 rounded-sm border border-default bg-muted p-2" data-testid="env-probe-report">
    ...
    <div v-if="report.operations.length" data-testid="env-probe-operations">
      ...
      <ul class="mt-0.5 space-y-1 text-2xs">
        <li v-for="(op, i) in report.operations" :key="i" class="flex items-start gap-1.5">
          ...
          />
          <span class="min-w-0">
            <span class="text-default">{{ op.name }}</span>
            <span v-if="op.authenticated" class="ml-1 text-dimmed">
