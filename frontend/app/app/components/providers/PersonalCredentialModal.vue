...
// start/retry of a Claude-pinned run needs it — driven by the personalSubscriptions
// store's `pending` state, which is set when the server replies 428 credential_required.
// On submit it transparently retries the gated action and caches the password.
...
const title = computed(() => {
  switch (pending.value?.reason) {
    ...
    case 'no_subscription':
      return 'Connect your Claude subscription'
    case 'subscription_expired':
      return 'Your Claude subscription expired'
    default:
      ...
      <div class="space-y-4">
        <template v-if="needsConnect">
          <p class="text-sm text-slate-400">
            This task uses a Claude model, which runs on <strong>your own</strong> Claude
            subscription. Connect (or renew) it first, then start the run again.
          </p>
          <div class="flex justify-end gap-2">
            <UButton color="neutral" variant="ghost" @click="open = false">Cancel</UButton>
            <UButton icon="i-lucide-shield-check" @click="goConnect()">Connect subscription</UButton>
          </div>
        </template>

        <template v-else>
          <p class="text-sm text-slate-400">
            Enter the personal password that protects your Claude subscription. It unlocks your
            credential for this run only and is cached in this browser so you won’t be asked again
            for a while.
          </p>
