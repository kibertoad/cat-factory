/**
 * The Nuxt UI theme editor's presets (https://ui.nuxt.com/theme), copied from
 * `docs/app/utils/theme/engine/presets.ts` in nuxt/ui (MIT). The editor collapses a share link
 * whose document is EXACTLY a preset to `{ version: 1, preset: '<id>' }`, so the importer needs
 * these documents to rebuild such a link. They are a DECODE TABLE, not menu entries: the switcher
 * offers the app's built-ins (`builtins.ts`), and a user who wants a preset imports its link.
 * Re-copy when the editor changes a preset; `theme.spec.ts` pins the ids the editor ships today.
 *
 * `icons` is carried for round-tripping and ignored on apply (the app ships one icon set).
 */
import type { StyleOptions, ThemeDoc } from '~/utils/theme/doc'
 ...
 * the way picking a neutral in the editor does (selectPalette's remaps), so a
 ...
 * reactive editor state, so a spread would hand six presets the same
...
export const presets: ThemePreset[] = [
  ...
  },
  {
    ...
    name: 'Mint',
    description:
      'Teal on an olive neutral, pill buttons and fields, chunky rounded type, large controls.',
    doc: {
      version: 1,
      colors: {
        primary: 'teal',
  ...
  },
  {
    ...
    name: 'Coral',
    description:
      'Rose on warm stone, cards floating on shadows, teal for success, neutral focus rings.',
    doc: {
      version: 1,
      colors: {
        primary: 'rose',
      ...
      components: {
        ...
        card: {
          compoundVariants: [
            { variant: 'outline', class: { root: 'ring-0 dark:ring shadow-xl shadow-black/5' } },
          ],
        },
      },
    },
  },
  {
    ...
    name: 'Sunset',
    description:
      'Orange on a warm taupe neutral, glowing actions, yellow secondary, subtle panels.',
    doc: {
      version: 1,
      colors: {
        primary: 'orange',
      ...
      components: {
        ...
        button: {
          compoundVariants: [
            { color: 'primary', variant: 'solid', class: 'shadow-md shadow-primary/30' },
          ],
        },
      },
    },
