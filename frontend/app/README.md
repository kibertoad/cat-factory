...
- **Never an arbitrary value** (`rounded-[10px]`) and **never a raw `border-radius` in px, rem or em**, the per-corner longhands (`border-top-left-radius`, the logical `border-start-start-radius`) and the JS `borderRadius` spelling a `:style` binding or an `element.style` assignment uses included. In a stylesheet reached from `main.css` use `var(--radius-xs|sm|md|lg|xl|2xl|3xl)`, one of the seven rebinds: `var(--radius-4xl)` and the bare `var(--radius)` are the same fixed literals behind a variable and are banned too. In a component's own scoped `<style>` block use `var(--ui-radius)` or `calc(var(--ui-radius) * N)` instead, because Nuxt UI declares the `--radius-*` scale in a `@theme default inline` block and Tailwind emits those variables only where the COMPILED stylesheet graph references them, which an SFC style block is not part of. A `rem` radius tracks the root font size, which is a different theme knob. The pill spellings (`9999px`, `50%`) and `0` are exempt, and a shorthand is read value by value, so `50% 50% 8px 8px` is still caught on the `8px`.
- `scripts/check-frontend-radius.mjs` (CI) bans all four. It reads a utility only where one can live, inside a `class=` attribute value, a single-quoted or template string, or after `@apply`, because `rounded` is also an ordinary English word and `alt="rounded avatar"` is not a class list. An interpolated step (`rounded-${size}`) is banned outright, being unresolvable and so never one of the seven. A line that genuinely needs a fixed radius says why with a `radius-literal-ok:` comment, on that line or the one above.
