...
- **Text over a `bg-primary` or alias fill is `text-inverted`**, not `text-white` (white-on-white under the Mono theme in dark) or `text-highlighted` (which follows the page, not the fill).
- **A `hover:` colour utility must differ from its resting value.** `text-primary … hover:text-primary` leaves no feedback; an alpha step (`bg-primary/90 … hover:bg-primary`) is a real hover. The guard bans all three of these too.
