...
- 3b89686: Say what to change when a step's tool server (MCP) was dropped, not only why.
...
- 55422e8: Show which document source the context picker searches, and stop asking a question the frame already answered
...
- 6293d33: Cover four essential product surfaces end to end (and instrument what they needed)
...
- cad3408: Attaching a document to a task is member-tier work, not integration management
...
- b0435e2: Add four tutorial tours for the platform behind the delivery loop, and keep the launch prompt to the first-run arc.
...
- af69370: Add a tutorial catalogue: every guided walkthrough the deployment ships, startable, resumable or
...
- af69370: Publish the parked gate's id on the initiative plan-review rail (`data-approval-id`), so a
...
- b05b8ff: Make a tutorial tour point at controls the user can actually see, reach by keyboard, and come
...
- b8d0a0c: Tutorial tours now cover the whole delivery loop, not just the board layout. Four new tours
...
- 0bc6505: In-app tutorial tours. On first launch the app asks whether the user wants a guided tour
...
- 7329a01: Let the review/reading overlays take the whole screen instead of floating in a 1024px card.
...
- 8b267bf: Move the basic/advanced interface-tier switcher to the top of the sidebar and make the choice
  visible at rest. Expanded, it is now a segmented control showing both tiers rather than a
  dropdown that only named the current one; in the collapsed rail it is a single button that flips
  the tier, labelled with the tier it is currently on.
...
- de129bc: Give the initiative plan-approval gate the step reader's layout instead of a card inside the tracker.
...
- b4f1370: Render the panel sections that were silently dropped, and stop an omitted optional query param from reaching the server as `key=`.
...
- 9ca6d20: Refresh the frontend dependency tree — direct and transitive — to the newest versions published
...
- 162e82e: Narrow basic interface mode to the everyday delivery loop. Repo bootstrap, Platform
  observability and Reports become advanced-tier nav destinations; the frame header's
  recurring-pipeline and initiative authoring buttons are advanced-only; and post-release
  health (the Integrations-hub row and the service inspector's monitor/SLO panel) reveals
  itself in basic mode only once the workspace has a connection or the frame has a mapping,
  so a live Datadog watch is never concealed by the tier.
...
- 3057db1: Make a selected initiative offer to delete the INITIATIVE (not "delete service"), and give the
...
- 6b91e97: Redraw the basic/advanced nav line by route count. A destination is now `advanced` only when
...
- 48cfe96: Order the requirements-review findings by what still needs a reaction: unanswered findings and
...
- 1c12289: Treat an EMPTY `apiBase` as the same-origin deployment topology when deriving the WebSocket
  origin.
...
- ddcdcd8: Make the environment self-test's "not provisionable" error specific and actionable.
...
- 1317c7b: Consumer extension mechanism, slice D: top-level overlays. A deployment extending the
...
- 1dc3643: Surface the agent's effort self-assessment in every result window, not just the generic step-detail panel. `ResultWindowShell` now renders `step.effortReport` as a collapsible footer (difficulty + what reduced the agent's effectiveness, expanding to the full report), resolved from the active result view, so the merger / tester / PR-review / structured-output / follow-up / fork-decision windows all show it without per-window code.
...
- 90a0c1b: PR-review UI + personal-password cache hygiene (ADR 0026 D2.2 + D7):
...
- 7d2a736: Surface the PR reviewer's live phase on the board and its run details in the review window.
...
- 7c4d68d: PR review window: make the "reviewing" progress precise instead of a static "Reviewing the
...
- 52196df: Adopt modular-react#83 (production-feedback items 3 & 4) in the environment-setup journey. Bump `@modular-frontend/core` (0.6.0), `@modular-frontend/journeys-engine` (1.9.0), `@modular-vue/core` (1.5.0) and `@modular-vue/journeys` (1.4.0), then drop the `defineModule` `as const` narrowing workarounds (item 3, literal inference) and derive the wizard's step order + a new "Step X of N" progress indicator from the annotated transition graph via `useJourneyProgress` / `resolveStepSequence` instead of a hand-maintained `ENV_STEP_ORDER` (item 4).

## 0.145.2

### Patch Changes

- c843377: Fix the PR deep-review window ("Review & approve") throwing `ReferenceError: can't access lexical declaration 'instanceId' before initialization` on every open, and remove the whole bug class at its source.
...
- 2138e45: The fragment library's "Documents" tab GitHub picker now stages multiple files
...
- b894628: Fix the task inspector's context attach: the "Context documents" and "Context
...
- 76ce300: Adopt modular-vue slice 5 (agent-run window chrome → `ResultWindowShell`): convert
...
- 70ea4a3: Adopt modular-vue in the Nuxt layer (slice 3: wizards → journeys, pilot). The
...
- 9207456: Adopt modular-vue in the Nuxt layer (slice 1: navigation manifest). The
...
- f86177d: Adopt modular-vue in the Nuxt layer (slice 0: registry in the layer). Wires a
...
- 0edeadc: Workspace RBAC (slice 9): SPA membership-management UI.
...
- f1e3e13: Workspace RBAC (slice 8): SPA read-side degradation for viewers/members.
...
- 84a561c: Add-service-from-repo: multi-select services from a monorepo in one pass. The
...
- c28f89e: Frontend startup optimizations (app-startup initiative, items 1/8/12):
...
- b34ab46: Give every pipeline-conflict toast a translated description and a jump action (error-message initiative G1).
...
- a735824: Mobile-friendly foundations (initiative slice A): safe areas, input font floor, theme color.
...
- 9b51578: Confirm a stored personal subscription inline instead of surfacing a validation error.
...
- 25ac984: Apriori branches (slice 4): a task's Run settings now expose the pre-existing branches
...
- fd16dc1: UX papercuts: guard content-heavy modals against silently discarding typed input, and make
...
- 41259cf: Initiative tracker: surface phase checkpoints in the SPA (slice 3 of the
...
- 8f5294c: Show a spinning loader (not a static question mark) on a requirements-review /
...
- 3885661: Fix the shared confirm dialog opening invisibly behind full-screen windows: `UModal` carries no
