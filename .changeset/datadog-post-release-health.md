...
- **No blind revert.** On a regression the gate dispatches an **`on-call`** container agent
  that clones the released PR head, correlates its diff with the regression evidence
  (alerting monitors/SLOs + recent error logs) and returns a JSON assessment
  (culprit confidence + `revert`/`hold`/`monitor` recommendation). It makes no commits and
  reverts nothing — the engine raises a **`release_regression`** notification for a human to
  decide.
...
- New harness route `POST /on-call`; the executor-harness image is bumped to `1.7.0`.
