...
handshake on a refusal that is not one of the three MCP-reserved error codes.
...
`state.groups[*].last_triggered_ts`, so the post-release-health gate can once again tell a standing
alert from one this release caused; the field it used to read is not in Datadog's schema, so the
transition time was silently always absent. Figma OAuth refreshes at `/v1/oauth/token`, which
...
Linear rate limits are read off the error `code`, because Linear answers an exhausted quota with
HTTP 400. The OTLP exporter reads `partialSuccess` instead of treating any 200 as full acceptance.
