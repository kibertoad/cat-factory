...
settings on entry and drains before every wait that hands the isolate, and its buffer, back
mid-run (`step.sleep`, `step.waitForEvent`, and a `step.do` attempt that threw into the step's
retry backoff) as well as in a `finally`.
