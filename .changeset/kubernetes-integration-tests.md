...
`apply-patch+json`, namespace `409` idempotency, Deployment readiness, and the
`status.loadBalancer` shape — were never validated against a real apiserver. A new
integration suite (`*.it.spec.ts`, run via `pnpm --filter @cat-factory/integrations
test:integration`) now drives both adapters against a real **k3d (k3s-in-Docker)** cluster,
asserting the pod-proxy round-trip and the k3s ServiceLB-assigned URL for real. It self-skips
when the `K8S_IT_*` cluster env is unset, and in CI runs as a blocking job gated behind a
paths filter so the k3d cluster only spins up when Kubernetes code changes.
