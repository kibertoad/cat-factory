...
human-test result views), the Pipeline builder, and the Model Configuration panel are now
capped to the dynamic viewport (`dvh`) so their content and controls stay reachable above
the mobile browser chrome instead of being clipped; the Pipeline builder's three columns
stack and scroll as one on compact viewports.
