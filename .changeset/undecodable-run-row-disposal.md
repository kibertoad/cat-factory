...
- **Isolation.** Both facades' sweeps now recover one run at a time inside a per-run boundary, log
  the run they skipped, and count it as `sweep.run_recovery_failed`. The Worker's `SweepResult`
  gains a `failed` tally.
...
- **A write-side guard.** Composing the stored `detail` for a run carrying no `blockId` now throws,
  so the writer that produces one reports the fault instead of a sweeper hours later. Both facades'
...
import breaks) because the engine has to be able to recognise it.
