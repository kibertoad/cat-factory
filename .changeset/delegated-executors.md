...
Internals: `AgentSurface` gains `delegated`, `AsyncAgentExecutor.reclaimRun` may return a `RunReclaimReport`, and `PipelineStep` gains `delegated`. Two new `error.details.reason` values, `delegated_executor_unwired` (409) and `delegated_executor_failed` (503), both additive.
