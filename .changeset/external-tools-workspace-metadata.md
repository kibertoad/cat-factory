...
- **A tool that cannot resolve stays LISTED and explains itself on click**, with `missing-metadata`
  (naming the unfilled fields), `unresolved` and `unsafe-url` as three separate causes. Hiding it
  would make an unconfigured workspace look identical to a deployment that never registered the
  tool — and the person reading the sidebar is usually the one who can fix it.
...
disagree with the app the moment either side is deployed alone.
