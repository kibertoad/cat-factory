...
custom-manifest-type contracts, a `provisioning` field on the service-frame `Block`, and
`provisionType`/`engine` fields on the environment handle. Introduces the per-user infra
handler override table (`environment_user_handlers`, local-mode) and the workspace
