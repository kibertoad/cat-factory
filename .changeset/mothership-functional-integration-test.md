...
- `@cat-factory/server`: widen `PILOT_PERSISTENCE_METHODS` to the run/board methods the path
  exercises, each workspace-scoped: `documentRepository.{listByBlock,get}`,
  `taskRepository.{listByBlock,get}`, `environmentRegistryRepository.{getByBlock,get}`, the run-start
  ...
  raise `notificationRepository.{findOpenByBlock,upsertOpenForBlock}`. Round-trip +
  cross-account-scope unit tests for each are added to `persistenceRpc.spec.ts`.
