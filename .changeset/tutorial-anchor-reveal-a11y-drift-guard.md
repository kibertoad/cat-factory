...
a step re-anchors when its control is replaced underneath it.
...
never on a click-to-advance step, where the app is opening a modal that owns focus. Step changes
are announced through a dedicated `role="status"` region, because a dialog whose contents are
replaced wholesale is not reliably announced. Ring and spinner motion sit behind `motion-safe:`,
and a reveal is instant under `prefers-reduced-motion`. Deliberately still not a focus trap: half
the catalog asks the user to operate the real control behind the card.
