...
already scaled Nuxt UI's components.

`common/SectionLabel.vue` becomes the one section eyebrow, replacing the six hand-written recipes
across 317 call sites. Its colour is `text-dimmed`, so the labels that were `text-muted` render a
step fainter than before. `scripts/check-frontend-type-scale.mjs` keeps the literal form from
returning.
