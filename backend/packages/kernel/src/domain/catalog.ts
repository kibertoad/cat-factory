...
export const DEFAULT_WORKSPACE_SETTINGS: WorkspaceSettings = {
  ...
  // Null, not the platform default's id: an unset value and a workspace that deliberately picked
  // the built-in preset behave identically, and only the null spelling lets a change to WHICH
  // preset is the platform default reach a board that never made a choice. That is exactly what
  // it bought when the default moved to `pl_bugfix_tested`
  // (`BugFishingController.resolveDefaultFixPipelineId`).
