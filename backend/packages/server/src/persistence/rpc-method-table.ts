// The mothership-mode persistence ALLOW-LIST: which repository methods may be invoked over the
// reflective `POST /internal/persistence` endpoint, and the {@link ScopeRule} that binds each one
// to an account so a call outside the machine token's scope cannot reach another tenant's data.
//
// Split out of `rpc.ts` (which owns the wire envelope + the dispatcher that ENFORCES this table)
// along the natural seam: this file is pure DATA plus the vocabulary describing it, and it is the
// half that grows — every slice that reaches a new repository from a mothership node adds rows
// here and nothing else. Keeping it separate means the dispatcher's control flow stays readable
// as the table keeps accreting, and a reviewer auditing "what can a node call" reads one file.
//
// `rpc.ts` re-exports everything here, so no call site imports this module directly.
 ...
 *   - `workspaceFieldList` — `args[arg]` is an ARRAY of records each carrying a `workspaceId`
 *                       field (a BATCH `upsertMany(records)`). EVERY record is bound like
 *                       `workspaceField`; one out-of-scope row refuses the whole call, so a
 *                       batch write can never smuggle a foreign row in behind legitimate ones.
...
export type ScopeRule =
  ...
  | { kind: 'workspaceFieldList'; arg: number }
...
export const REMOTE_PERSISTENCE_METHODS: PersistenceMethodTable = {
  ...
  // The per-service ACCEPTANCE CRITERIA, keyed by service-frame block like
  // `validationConfigRepository` above. Nothing sealed — criteria are product knowledge a member
  // curates — so the plain records cross the machine API. The inspector CRUD
  // (`get`/`listByWorkspace`/`delete`/`deleteByBlock`), the dispatch's frame read
  // (`listByFrameBlocks`) and the accretion write (`upsertMany`) are all workspace-scoped on
  // arg0, except the record-based `upsertMany`, which binds on EVERY record's `workspaceId`
  // field (a batch write must not be able to smuggle one foreign row through on the back of
  // legitimate ones).
  acceptanceCriterionRepository: {
    get: { scope: { kind: 'workspace', arg: 0 } },
    listByFrameBlocks: { scope: { kind: 'workspace', arg: 0 } },
    listByWorkspace: { scope: { kind: 'workspace', arg: 0 } },
