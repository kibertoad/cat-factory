...
import {
  ...
  checkWorkspaceFieldListScope,
} from './rpc-scope.logic.js'
import {
  REMOTE_PERSISTENCE_METHODS,
  type PersistenceMethodTable,
  type ScopeRule,
} from './rpc-method-table.js'

// The per-repo method allow-list + its scope vocabulary live in `rpc-method-table.js` (pure data;
// see that module's header for why). Re-exported here so every existing importer of `rpc.js` —
// the controller, the client proxy, the allow-list completeness test — is unchanged.
export {
  REMOTE_PERSISTENCE_METHODS,
  type MethodSpec,
  type PersistenceMethodTable,
...
async function checkCallScope(
  ...
  switch (rule.kind) {
    ...
    case 'workspaceFieldList': {
      // The batch mirror of `workspaceField` — extracted like the other multi-step record checks
      // so this switch stays under its complexity ceiling.
      const denial = await checkWorkspaceFieldListScope(args[rule.arg], opts, inScope, denied)
