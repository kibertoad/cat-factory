...
import { imageVariantFor, providerOf, refForHandle, stepJobId } from './containerJobAddressing.js'
...
export class ContainerAgentExecutor implements AsyncAgentExecutor {
  ...
  async stopJob(handle: AgentJobHandle): Promise<void> {
    await this.jobs.release(handle.workspaceId, refForHandle(handle, this.agentKindRegistry))
  }
