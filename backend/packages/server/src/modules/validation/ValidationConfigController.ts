...
/** Resolve the validation-config service or raise a 503 — it isn't wired on this deployment. */
function requireValidationConfig<E extends AppEnv>(c: Context<E>): ValidationConfigService {
  const validationConfig = c.get('container').validationConfig
  if (!validationConfig) throw new UnavailableError('The validation-check store is not configured')
