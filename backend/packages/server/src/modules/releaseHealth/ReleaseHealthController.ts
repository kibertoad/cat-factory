...
/** Resolve the release-health module or raise a 503 — it isn't wired on this deployment. */
function requireReleaseHealth<E extends AppEnv>(c: Context<E>): ReleaseHealthModule {
  const releaseHealth = c.get('container').releaseHealth
  if (!releaseHealth) throw new UnavailableError('The observability integration is not configured')
