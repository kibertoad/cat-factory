...
/** The `userSecrets` capability, or a 503 — this deployment wired none. */
const requireUserSecrets = <E extends AppEnv>(c: Context<E>) =>
  requireCapability(c.get('container').userSecrets, 'User secret storage is not configured')

/** The signed-in user, or a 401 naming the action. */
const requireSignedIn = <E extends AppEnv>(c: Context<E>) =>
  requireUser(c, 'Sign in to manage your secrets')
