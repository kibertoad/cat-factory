...
/** The `localModelEndpoints` capability, or a 503 — this deployment wired none. */
const requireLocalModelEndpoints = <E extends AppEnv>(c: Context<E>) =>
  requireCapability(
...
export function localModelEndpointController(): Hono<AppEnv> {
  ...
  buildHonoRoute(app, listLocalModelEndpointsContract, async (c) => {
    const local = requireLocalModelEndpoints(c)
