...
/** The `workspaceMemberService` capability, or a 503 — this deployment wired none. */
const requireWorkspaceMemberService = <E extends AppEnv>(c: Context<E>) =>
  requireCapability(
...
export function workspaceMemberController(): Hono<AppEnv> {
  ...
  buildHonoRoute(app, listWorkspaceMembersContract, async (c) => {
    const service = requireWorkspaceMemberService(c)
