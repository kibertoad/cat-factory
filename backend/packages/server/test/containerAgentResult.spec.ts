...
// Characterisation tests pinning the runner-output → engine-result normalisation. They lock in
// the conservative coercions (merge/on-call/test) and the structural PR / pushed branches, and
// they are what proved the move of those coercions off an `agentKind === …` chain in the executor
// and onto each built-in's own `mapStructuredResult` registration: byte-for-byte the same answers,
// now resolved through the registry a deployment's kinds register on.
const registry = defaultAgentKindRegistry()
const result = (over: Partial<RunnerJobResult>): RunnerJobResult => over as RunnerJobResult
const run = (over: Partial<RunnerJobResult>, kind?: string) =>
  toRunResult(result(over), kind, registry)

describe('toRunResult', () => {
  it('surfaces an opened PR structurally (PR wins over pushed)', () => {
    const r = run({ pushed: true, prUrl: 'https://github.com/o/r/pull/42', branch: 'feature/x' })
  ...
  it('maps an in-place pushed job to a sensible output', () => {
    expect(run({ pushed: true }).output).toBe('Pushed changes to the branch.')
    expect(run({ pushed: false }).output).toBe('No changes were produced.')
  })

  it('passes a registered custom kind through as custom', () => {
    const r = run({ custom: { foo: 'bar' }, summary: 'done' }, 'some-custom-kind')
  ...
  it('honours the spec-writer noBusinessSpecs flag (no spec committed)', () => {
    const r = run({ custom: { noBusinessSpecs: true } }, SPEC_WRITER_AGENT_KIND)
  ...
  it('leaves blueprintService unset for a nameless/garbage tree', () => {
    const r = run({ custom: { not: 'a tree' } }, BLUEPRINTS_AGENT_KIND)
  ...
  it('coerces a garbage merger assessment to the conservative 1/1/1 default', () => {
    const r = run({ custom: null, summary: 'looked at it' }, MERGER_AGENT_KIND)
  ...
  it('coerces a garbage on-call assessment to confidence 0 / hold', () => {
    const r = run({ custom: {}, summary: 'investigated' }, ON_CALL_AGENT_KIND)
  ...
  it('honours a tester greenlight only when no blocking concern is open', () => {
    const passing = run({ custom: { greenlight: true, concerns: [] } }, TESTER_AGENT_KIND)
  ...
  it('forces a tester greenlight off when the run aborted', () => {
    const r = run(
      { custom: { greenlight: true, abort: { reason: 'env never came up' } } },
      TESTER_AGENT_KIND,
    )
