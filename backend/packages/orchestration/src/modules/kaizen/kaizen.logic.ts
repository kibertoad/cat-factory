...
/**
 * `agentKind|model|promptVersion[|vID][|wN]` — the key a verified combo is tracked under.
 *
 ...
 *
 * - `vID` is the deployment's registered agent-kind VARIANT the step ran as
 *   (`stepOptions.agentVariantId`). A variant runs the base kind under different text, so
 *   sharing a key with the stock kind would let one's gradings verify the other.
...
export function comboKeyFor(
  ...
  const varied = agentVariantId === undefined ? base : `${base}|v${agentVariantId}`
