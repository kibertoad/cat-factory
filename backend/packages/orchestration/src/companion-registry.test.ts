...
// The COMPANION half of the installation-level extension seams: a deployment registering its own
// REWORK PAIR (a producer, and a companion that grades it and loops it back below the bar).
// Its own file rather than another block in `extension-registries.test.ts`, which the pair's
// tests pushed past the file-size ratchet — and companions are a cohesive concern, so the split
// falls on a real seam rather than an arbitrary line.

describe('companion registry', () => {
  // A deployment's own REWORK PAIR: a producer, and a companion that grades its output and
  // loops it back below the bar. Before this seam the only way to say that was a judge, which is
  // a different machine — a judge scores against a rubric and disposes, where a companion drives
  // the producer's own rework budget.
  ...
  it('accepts a registered pair in a pipeline, and still enforces the adjacency rule', () => {
    ...
    // Immediately after its target: valid, exactly as `coder` → `reviewer` is.
    expect(() =>
