 ...
 * and re-exposed on `Core` for the HTTP snapshot. `workRunner`/`executionEventPublisher`/`logger` fall
 * back to no-ops, and `caches` (the caching-initiative slice bag) to bare in-memory loaders — so the
...
export function resolveCoreRuntime(dependencies: CoreDependencies) {
  return {
    ...
    // Resolved once so every service is threaded the SAME instance and none of them has to
    // null-check: a facade that wired no logger gets the no-op, never `undefined`.
