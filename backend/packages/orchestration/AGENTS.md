...
- `modules/useCases/`: the public INLINE USE-CASE surface, the non-container sibling of a reusable
  ...
  workspace budget guard) and `LlmInlineUseCaseGenerator` is the `BugHuntAssessorService`-shaped
  ...
  outside its declared bounds is refused rather than clamped. See `backend/docs/inline-use-cases.md`.
