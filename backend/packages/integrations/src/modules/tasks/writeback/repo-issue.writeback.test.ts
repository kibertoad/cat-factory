...
function installations(row: { provider: string } | null): GitHubInstallationRepository {
  return {
    getByWorkspace: async () => (row ? ({ installationId: 42, ...row } as never) : null),
  } as unknown as GitHubInstallationRepository
}

const CTX = { workspaceId: 'ws_1', credentials: {} }
