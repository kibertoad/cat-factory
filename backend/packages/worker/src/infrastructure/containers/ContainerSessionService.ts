...
// provider API key. The proxy verifies the token and forwards to the
// token-locked provider/model, so a container cannot pick an arbitrary (more
// expensive) model or call the proxy for a different workspace. Built on the
// same HMAC primitive as the auth session token (AUTH_SESSION_SECRET).
