...
export const CONFLICT_REASONS = [
  ...
  // An AGENT DRY RUN was requested on a deployment that cannot drive one: no container runner, no
  // model the LLM proxy can serve, or no repository seam for the prober's checkout. Refused BEFORE
  // anything is provisioned, because a run admitted without a prober would stand an environment up
  // and then wait for a stage nothing can advance. Its own reason rather than the generic 503 copy:
