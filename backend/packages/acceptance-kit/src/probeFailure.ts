...
//
// A prerequisite reaches the deployment over HTTP, so a probe fails in one of three fundamentally
// different ways, and a gate that renders all of them as the same sentence names none of them.
//
...
//
// What the kit cannot know is the suite half of each remedy: which variable holds the base URL,
// which file it was typed into, how a pass is resumed. Those ride the optional `SuiteIdentity`, and
// an absent one costs the sentence rather than the diagnosis.
//
...
function apiSteps(
  ...
  if (error.status === 404 && error.code === 'unknown') {
    return [
      ...
      'That is what a deployment OLDER than the suite driving it looks like. A suite drives ' +
        '`/api/v1` operations declared by the SDK it is built against, so a deployment running an ' +
        'earlier build serves none of the newest. Rebuild and restart it.',
  ...
  if (error.status === 401 || error.status === 403) {
    return [
      ...
      'A token\'s scope is fixed when it is created: mint a new one with scope "Full access" ' +
        '(Integrations, "API access tokens") rather than trying to raise this one, and configure ' +
        'the suite with it.',
    ]
  }
  if (error.status >= 500) {
 ...
 * the API key, and saying so is most of the value: the remedy this replaced sent readers to inspect
 * a token that was never sent. What a status DOES narrow is which layer answered, since
...
function rootReadSteps(
  ...
  const steps = [
    `${request} answered ${error.status}. This read carries no credential by design, so nothing ` +
      `about the API key is implicated: a cat-factory backend serves this route on every boot it ` +
      `survives, so what answered is either not one or is not the layer that would.`,
  ]
  if (error.status === 404) {
    steps.push(
  ...
  steps.push(baseUrlStep(target, identity))
  return steps
}
...
function foreignFailure(
  ...
  return {
    ...
    remedy: buildRemedy(foreignSteps(target, identity), target, identity),
  }
}
...
export function describeProbeFailure(
  ...
  if (error instanceof CatFactoryApiError) {
    ...
    return {
      ...
      remedy: buildRemedy(
        [...apiSteps(error, probe.target, identity), ...requestIdStep(error)],
  ...
  if (error instanceof DeploymentAnswerError) {
    if (error.answer === 'refused') {
      return {
        ...
        status: error.status,
        detail: detailOf(error) || `HTTP ${error.status}`,
        remedy: buildRemedy(rootReadSteps(error, probe.target, identity), probe.target, identity),
      }
    }
    return foreignFailure(error, probe.target, identity)
  }
  ...
  if (error instanceof CatFactoryDecodeError) return foreignFailure(error, probe.target, identity)
  ...
  return {
    steps: [
      ...
      'Fix that, then re-run the suite.' +
        (identity
          ? ` A pass that has already created things resumes when ${identity.runIdVariable} is ` +
            `set to the run id printed above this failure; only a pass that created nothing yet ` +
            `starts clean.`
          : ''),
    ],
    ...(reachTarget
...
export function probeFailureVerdict(
  ...
  const failure = describeProbeFailure(error, probe, identity)
