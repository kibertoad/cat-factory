...
export type WaitOptions<T> = {
  ...
  /**
   * What a suite adds under BOTH failure messages: what is still standing, and how to pick it up.
   *
   * Injected rather than written here, and for the same reason the tolerance is: this module is the
      ...
      if (outageMs >= tolerate.graceMs) {
        ...
        throw new Error(
          formatOutage({
            label,
            outage,
            lastState: lastAnswer,
            outageMs,
            graceMs: tolerate.graceMs,
            epilogue,
          }),
        )
      }
 ...
 */
export function formatExpiry(options: {
  label: string
  lastState: string
  elapsedMs: number
  budgetMs: number
 ...
 */
export function formatOutage(options: {
  label: string
  outage: string
  lastState: string
  outageMs: number
  ...
  const { label, outage, lastState, outageMs, graceMs, epilogue } = options
  return (
