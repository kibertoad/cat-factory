...
Even with the gate removed, the fallback alone is weak for this shape: `SliceTracker.progress()` (`subagents.ts:92-105`) derives `completed` from `Task` tool_results, and parallel subagents all return in a burst at the end, so it reports 0/N until the finish then jumps to N/N. D2.1 restored slice *items* but cannot produce an incremental percentage for the parallel shape.
