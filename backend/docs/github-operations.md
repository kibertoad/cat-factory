...
> ([GitHub App](https://www.catfactory.ai/deploy/github-app.html)); the hardening that goes with
> them is [`security-model.md`](./security-model.md)'s operator checklist. What stays here is the
> runbook for someone with the repo open: the token plumbing, the failure signatures, and the
> recovery steps.
