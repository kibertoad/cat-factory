...
- **Contract**: `pipelineSchema` (`@cat-factory/contracts` `src/entities.ts`) gains
  ...
  migration/back-fill; existing rows read as unrestricted.
...
- **Comment** (all vendors): "Taken by cat-factory" + the run/board link, via the existing
...
- **Backwards compatibility**: pre-1.0 rules apply, so `availability` and `issueIntake` are
