...
- **Surface-specific codes**, unique to `/api/v1` (branch on these, not on the message):
  ...
  | `pipeline_required`              | 400     | `POST /tasks/:id/start`: no `pipelineId`, no pinned pipeline, and the workspace declares no unattended default                                                                                 |
...
| `POST /api/v1/tasks/:taskId/start`               | `write`¹ | Run it. Start the task's run, falling back to its pinned pipeline, then to the workspace's default pipeline for a run nothing is watching (`400 pipeline_required` with none of the three). `202` with the task projection.                                                                                                                                           |
