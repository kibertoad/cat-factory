...
that let a native adapter (e.g. one for an in-house ephemeral-environment system) manage the **provider's
configuration file inside the deployed repo**: validate it, mechanically generate it, and (as a
safety net) have a coding agent repair it. It complements [`native-environment-adapter.md`](./native-environment-adapter.md),
...
> wired, and tested.** Only the native adapter itself (Part B, a separate repo) remains; see
> [What's left](#whats-left).

## Why this exists

Some ephemeral-environment systems require a **config file in the target repo** before they can
provision. The motivating case is an in-house ephemeral-environment system: to spin up a PR
environment (or a Tester "sandbox"), it reads a config file from the repo at provision time:

- A config file (say **`.acme-envs.yml`**) at the repo root, with a provider-defined schema.
...
- Tester instances additionally use an optional **`sandbox`** block with its own rules
  (setup/dev commands, a health path that must start with `/`, a port in 1–65535, bounded
  durations).

Critically, **that provider has no validate / dry-run endpoint**: it only reads the config file
from the VCS host internally, _during provision_. So a missing or malformed config fails
**asynchronously** (the environment lands in `status: failed`) with no early, actionable signal. These
...
- **Write target**: the default path commits straight to the target branch (the ref the provider
  will read, e.g. the PR head branch it provisions from). `openPr: true` commits to a
...
### Part B: the native adapter (separate repo, blocked on a release)

The adapter that _implements_ these three methods lives in the wrapper repo
(the provider-adapter package in `cat-factory-wrapper`) and is **blocked on a
`@cat-factory/kernel` release** carrying the new port members. Once released and the adapter is
bumped:

- `validateRepo`: read the provider's config file, YAML-parse, emit errors for empty
...
- `describeBootstrapInputs` + `bootstrapProviderConfiguration`: declare the variables (service
  ...
  parametrized config file; return `needsAgent: true` when an existing config conflicts in a way
  templating can't safely merge.
- `describeRepairAgent`: a prompt instructing the agent to make the config file conform to the
  provider's schema, referencing the supplied issues.
