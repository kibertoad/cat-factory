...
- **`agent_context_snapshots`**: the complete context an agent was PROVIDED per dispatch,
  ...
  available", which is why that message names no cause. The repo BOOTSTRAPPER files one, through
  the same module's second projection: it has no `AgentRunContext` (there is no block, no
  pipeline and no resolved fragments before the service exists), but it has a composed system
  prompt, a human's brief and the repositories it was pointed at, which is the whole of what the
  question means for that run.
...
the panel's grouping distinguish the survey's spend from the apply's.
