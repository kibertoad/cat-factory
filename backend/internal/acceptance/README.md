...
| `src/reset.ts`               | Starting over, suite half: which frames a clear targets (this configuration's two questions), what it cannot free, what it could not read, and what it does not reclaim. The plan/apply machinery under it is the kit's `reset.ts`. |
