// Spec 04: somebody outside the team files an issue, and the platform delivers it and closes it.
//
// The loop this spec exists for is the one a deployment with nobody in the app runs on, end to end
// and with nothing faked:
//
...
//
// ## Why it is a separate spec rather than another case in spec 02
//
// Spec 02 asks whether a brief this suite wrote can be delivered. This one asks whether work that
// arrived from OUTSIDE can be, and the difference is entirely in the parts spec 02 does not touch:
// the ticket import, the linked-issue context, and the writeback. Those are what a headless
...
// machinery spec 02 already grades: CI went green, the merger resolved an outcome. What is new is
// the pair `checkIssueWriteback` grades, and it is read from the PROVIDER rather than from the
// platform's own report: a closed issue, and the platform's two comments on it. `src/issueIntake.ts`
// explains why the second is what makes the first mean anything.
//
...
// ## Order
//
// Last, and after spec 03 for one reason: it delivers onto the BACKEND repository, which spec 02
// merged into and spec 03 does not touch (its fix belongs to the frontend). Running before spec 03
// would put two open pull requests on different repositories, which is fine, and two runs against
// the same default branch, which is the collision the ordering exists to avoid.

import type { PrReportRunProvider } from '@cat-factory/sdk'
import { beforeAll, describe, expect, it } from 'vitest'
import { assertChecks, checkCi, checkMergeDecision, checkNotTruncated } from '../src/evidence.ts'
import { offsetValidationIssue } from '../src/instructions.ts'
...
import { ISSUE_SOURCE_BY_PROVIDER, issueTarget, slug } from '../src/vcsIssues.ts'
import { assertPrerequisites, harness, issueApiFor } from './fixtures.ts'
...
describe('tracker intake: an outside issue is delivered and closed', () => {
  ...
  it('files an issue on the backend repository as an outside reporter would', async () => {
    ...
    if (!connection) {
      ...
      throw new Error('The workspace has no VCS connection, so there is no provider to file on.')
    }
    ...
    // The URL is what the task is linked THROUGH (`ticket.ref` takes a canonical key or a full issue
    // URL), so a record without one is a pass that cannot continue.
    expect(record.url, 'the filed issue has no web URL, so nothing can be linked to it').toContain(
      `${target.repo}`,
    )
    journal.finishPhase(`the reporter's issue is open at ${record.url}`)
  })

  it('files a task FROM the issue and drives pl_build to a merged delivery', async () => {
    ...
    const { run, record } = await fileAndDrive({
      ...
      // it, links it to the new task and re-reads it live for every agent step; a ref it cannot
      // resolve refuses this call rather than leaving an unlinked task behind, so a 201 IS the
      // claim that the link exists. The title and description are the caller's own framing, exactly
      // as an integration would send them, and stay short because the issue carries the detail.
      createTask: () =>
  ...
  it('gated the delivery on real CI and resolved a merge outcome', async () => {
    ...
    // The same machinery claims spec 02 makes, and no environment claim: `pl_build` deploys, but
    // what this spec is about is the intake and the writeback, and asserting the cluster legs again
    // would make an unrelated k3s wobble read as a broken tracker loop.
  ...
  it('closed the reporter’s issue and recorded the outcome on it', async () => {
    const issue = world.require('intakeIssue')
    const delivery = world.require('issueDelivery')
