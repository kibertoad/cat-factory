...
// ledger (`world.ts`), the journal (`journal.ts`), and the status CLI, which built both paths a
// third time. A fourth spelling is a pass an operator cannot find.
//
// It knows the NAMES and the write times, never the contents: `world.ts` reads a ledger and
// `status.ts` reduces a journal. That separation is what lets `findMostRecentPass` answer for a
// pass which recorded no fact at all, and that pass is the common one: an attempt a prerequisite
// refused writes a journal saying why and never opens a ledger.
//
...
// contents say when they disagree is `WorldStore`'s business, and it refuses.
...
/**
 * The pointer to the most recent pass that CREATED something, so `ACCEPTANCE_RUN_ID=latest` has
 ...
 * services are why the refusal fired, so the `ACCEPTANCE_RUN_ID=latest` the refusal itself printed
...
export function latestPointerPath(stateDir: string): string {
  return join(resolveStateDir(stateDir), LATEST_POINTER)
}
...
/**
 * The pass that ran most recently, whether or not it created anything: what `status` reports when
 * an operator names no run id.
 *
 * NOT the `latest` pointer, deliberately. That names the pass worth RESUMING, and the pass someone
 * asks `status` about is the attempt they just watched fail, which is by construction the one that
