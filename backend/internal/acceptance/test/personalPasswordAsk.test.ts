...
function harness(overrides: Partial<Parameters<typeof askForPersonalPassword>[0]> = {}) {
  const logs: string[] = []
  // What the HOLDER was PRIMED with, which is the whole observable effect: nothing hands a password
  // back any more (the ask fills the unlock's own closure), so a test watches the holder rather than a
  // value. Primed and not held, so the header still starts riding requests at the first `428`.
  const primed: string[] = []
  const deps = {
    log: (message: string) => logs.push(message),
    readPinned: async () => pinnedOn(),
    prime: async (_reason: string) => {
      primed.push('hunter2')
    },
    ...overrides,
  }
  return { deps, logs, primed, said: (text: string) => logs.some((line) => line.includes(text)) }
}

describe('askForPersonalPassword', () => {
  it('asks once and leaves the password in the holder for the whole pass', async () => {
    const { deps, primed } = harness()

    await askForPersonalPassword(deps)

    expect(primed).toEqual(['hunter2'])
  })

  it('names the preset and the model, so the prompt says what it is spending', async () => {
    const prime = vi.fn(async (_reason: string) => {})
    await askForPersonalPassword(harness({ prime }).deps)

    expect(prime).toHaveBeenCalledTimes(1)
    const reason = prime.mock.calls[0]?.[0] ?? ''
  ...
  it('continues the pass when there is no terminal to ask on', async () => {
    const { deps, primed, said } = harness({
      prime: async () => {
