...
export function defineExecutionTesterConformance(harness: ConformanceHarness): void {
  describe('execution engine', () => {
    registerTesterPipelineTests(harness)
    registerLocalInfraTesterTests(harness)
    registerVisualConfirmationTests(harness)
    registerTesterVerdictTests(harness)
  })
}

/**
 * Task pipelines through auto-merge, the Tester's fixer + quality-control loops, its
 * docker-compose stand-up record, and the frontend/visual pipeline gating.
 *
 ...
 */
function registerTesterPipelineTests(harness: ConformanceHarness): void {
  ...
  it('persists task agent-config and surfaces the contribution catalog', async () => {
    ...
    const created = await app.call<Block>('POST', `/workspaces/${wsId}/blocks/mod_sessions/tasks`, {
...
/**
 * The runtime-conditional Tester paths: the ones a mothership node (no local db) or the
 * Worker cannot serve, skipped per harness rather than asserted differently.
 *
 * Registered from the suite above; split out purely to keep each function within the
 * per-function line budget. Every test is unchanged.
 */
function registerLocalInfraTesterTests(harness: ConformanceHarness): void {
  it.skipIf(harness.name === 'mothership')(
    ...
    async () => {
      ...
      const task = await app.call<Block>('POST', `/workspaces/${wsId}/blocks/blk_frontend/tasks`, {
...
/**
 * The visual-confirmation gate: the binary-storage precondition, pass-through, park →
 * approve, and request-fix → re-park → approve.
 *
 * Registered from the suite above; split out purely to keep each function within the
 * per-function line budget. Every test is unchanged.
 */
function registerVisualConfirmationTests(harness: ConformanceHarness): void {
