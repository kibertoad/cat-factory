...
export function defineMiscConformance(harness: ConformanceHarness): void {
  describe(`[${harness.name}] conformance`, () => {
    ...
    registerSlackAndIdentityTests(harness)
 ...
 */
function registerRecurringIntakeTests(harness: ConformanceHarness): void {
...
/**
 * On-demand schedules, the workspace tracker selection, and the per-task / workspace
 * question-writeback toggles.
 *
 * Registered from the suite above; split out purely to keep each function within the
 * per-function line budget. Every test is unchanged.
 */
function registerTrackerWritebackTests(harness: ConformanceHarness): void {
  it('persists an on-demand schedule (no cadence) and fires it via run-now', async () => {
    ...
    const snapshot = await app.call<WorkspaceSnapshot>('GET', `/workspaces/${wsId}`)
    expect(snapshot.body.recurringPipelines?.find((s) => s.id === created.body.id)?.onDemand).toBe(
      true,
    )
  ...
  it('reads and writes the workspace tracker selection', async () => {
    ...
    const linear = await app.call<TrackerSettings>('PUT', `/workspaces/${wsId}/tracker-settings`, {
      tracker: 'linear',
  ...
  it('round-trips the per-task writeback overrides on a block', async () => {
    ...
    const patched = await app.call<Block>('PATCH', `/workspaces/${wsId}/blocks/${task.body.id}`, {
    ...
    const cleared = await app.call<Block>('PATCH', `/workspaces/${wsId}/blocks/${task.body.id}`, {
...
/**
 * The pipeline purpose classifier plus the bug-triage investigation / clarification and
 * reproduction phases.
 *
 * Registered from the suite above; split out purely to keep each function within the
 * per-function line budget. Every test is unchanged.
 */
function registerBugTriagePhaseTests(harness: ConformanceHarness): void {
...
/**
 * The Slack integration surface and the identity / onboarding flows.
 *
 * Registered from the suite above; split out purely to keep each function within the
 * per-function line budget. Every test is unchanged.
 */
function registerSlackAndIdentityTests(harness: ConformanceHarness): void {
  describe('slack', () => {
  ...
  describe('identity & onboarding', () => {
    ...
    it('resolves ONE personal account under concurrent first sign-in (no duplicate-key race)', async () => {
      ...
      const owner = await ob.users.findOrCreateByIdentity('github', `race_${crypto.randomUUID()}`, {
        name: 'Race Owner',
      })
