...
/**
 * Environment provider + backend connection registration (Kubernetes / Docker Compose /
 * the built-in Cloudflare Workers preview), and the async container-backed deploy lifecycle.
 *
 ...
 */
function registerEnvironmentBackendTests(harness: ConformanceHarness): void {
  ...
  registerDeployLifecycleTests(harness)
}

/**
 * Per-type infra handlers, the workspace custom-manifest-type catalog, the `infraless` and
 * `library` deployer no-ops, and native provider repo-config validation.
 *
 * Registered from the suite above; split out purely to keep each function within the
 * per-function line budget. Every test is unchanged.
 */
function registerInfraHandlerTests(harness: ConformanceHarness): void {
...
/**
 * Service-provisioning detection (deployment conventions + a registered provider's own
 * `detect()` hook) and the env-config-repair run.
 *
 * Registered from the suite above; split out purely to keep each function within the
 * per-function line budget. Every test is unchanged.
 */
function registerProvisioningDetectionTests(harness: ConformanceHarness): void {
  ...
  it('drives an env-config-repair run to success and records the post-repair validation', async () => {
    ...
    const { app, wsId, jobId, store } = await startEnvConfigRepairRun(harness)
  ...
  it('reads a stopped repair run’s structured failure back off the store', async () => {
    // The repair repositories decode `agent_runs.failure` through the shared, FULL-schema
    // `parseStoredAgentFailure`, which drops any record the wire contract wouldn't accept.
    // So a drift between what `EnvConfigRepairService.buildFailure` WRITES and what
    // `agentFailureSchema` requires costs every repair failure its diagnostics SILENTLY —
    // the infra window showing a stopped run with no reason — instead of failing loudly.
    // Driving a real stop and re-reading pins the write↔read pair on D1 and Postgres alike.
    const { app, wsId, jobId } = await startEnvConfigRepairRun(harness)

    const stopped = await app.call('POST', `/workspaces/${wsId}/agent-runs/${jobId}/stop`)
    expect(stopped.status).toBe(200)

    // Re-READ off the store: the stop response is the service's own in-memory patch, so only
    // a fresh snapshot exercises the repository's decode.
    const after = await app.call<WorkspaceSnapshot>('GET', `/workspaces/${wsId}`)
    const job = after.body.envConfigRepairJobs?.find((j) => j.id === jobId)
    expect(job?.status).toBe('failed')
    expect(job?.failure).toBeTruthy()
    expect(job?.failure?.kind).toBe('cancelled')
    ...
    expect(typeof job?.failure?.hint).toBe('string')
    expect(typeof job?.failure?.occurredAt).toBe('number')
  })
}

/**
 * Dispatch a real `env-config-repair` run and hand back the handles its tests drive.
 *
 ...
 */
async function startEnvConfigRepairRun(harness: ConformanceHarness): Promise<{
  app: ConformanceApp
  wsId: string
  jobId: string
  ...
  expect(started.body.repairJobId).toBeTruthy()

  return { app, wsId, jobId: started.body.repairJobId!, store }
}

/**
 * Board mutations against the same store: frames, modules, tasks, idempotent deletes, and
 * the archive/restore path for a service with unfinished tasks.
 *
 * Registered from the suite above; split out purely to keep each function within the
 * per-function line budget. Every test is unchanged.
 */
function registerBoardMutationTests(harness: ConformanceHarness): void {
  describe('board', () => {
...
/**
 * A deployer EnvironmentProvider failure surfacing as an `environment` run failure, and the
 * async container-backed deploy lifecycle driven to an identical environment on every facade.
 *
 * Registered from the suite above; split out purely to keep each function within the
 * per-function line budget. Every test is unchanged.
 */
function registerDeployLifecycleTests(harness: ConformanceHarness): void {
