...
/**
 * The requirements-review endpoints (re-review / async incorporate / async recommend) and
 * the implementation-fork decision loop, including its chat surface.
 *
 ...
 */
function registerRequirementsGateTests(harness: ConformanceHarness): void {
...
/**
 * Spec-increment ingest, estimate-gated step skipping, the requirements/clarity
 * substitutions that replace a block's description in every step, initiative-preset
 * steering (built-in and injected), and run restart from a chosen step.
 *
 * Registered from the suite above; split out purely to keep each function within the
 * per-function line budget. Every test is unchanged.
 */
function registerSpecIngestAndSteeringTests(harness: ConformanceHarness): void {
...
/**
 * The review store's own optimistic concurrency (race-audit 2.5), asserted at the
