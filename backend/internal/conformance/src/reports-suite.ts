...
export interface ReportsSeed {
  ...
  /** Insert a `services` row pointing at its frame block (the service's display name). */
  service(id: string, accountId: string, frameBlockId: string): Promise<void>
...
export function defineReportsSuite(
  ...
  describe(`[${name}] reports repository parity`, () => {
    ...
    async function seedFixture() {
      ...
      return { account, ws, wsB, svcOne, svcTwo, other }
    }
