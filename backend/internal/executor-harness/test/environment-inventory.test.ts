...
describe('rendering the inventory', () => {
  ...
  it('states what a container started HERE can reach, which running one does not settle', () => {
    ...
    expect(cut).toContain('has NO network')
    ...
    const unsure = line({ status: 'undetermined', reason: 'the egress check did not run' })
    expect(unsure).toContain('was NOT established (the egress check did not run)')
    expect(unsure).not.toContain('has NO network')
  })
