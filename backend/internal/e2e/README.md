...
| `result-window-shell.spec.ts`   | Slice 5 of the modular-vue adoption: the shared `ResultWindowShell` that all agent-run result windows now render inside. Drives the pilot (`MergerResultView`) — reach a low-confidence merger step, open it from the inspector run panel, assert the window mounts in the shell (`result-window` + the merger verdict body), and that it closes on all three paths the shell now owns via the upstream `useModalBehavior`: the close button, **Escape** (the shared overlay stack, not a per-window listener), and a backdrop click. A second test drives a converted `#header-extras` window (the CI `gate`) through the same shell — opening it from the run's step list, asserting its status badge renders in the shell's extras slot, and that the `manageEscape: false` handoff closes it on Escape — so the coverage isn't pilot-only. |
