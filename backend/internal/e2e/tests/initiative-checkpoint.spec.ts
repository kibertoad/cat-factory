...
test.describe('initiative phase checkpoint', () => {
  // Drives a full planning run (interviewer → analyst → planner → gate → committer) then two loop
  // waves with a pause in between — many durable pg-boss steps — so give it the slow budget.
  test.slow()
