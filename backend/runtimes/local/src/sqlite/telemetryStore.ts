...
CREATE TABLE IF NOT EXISTS llm_call_metrics (
  ...
  -- The phase/turn attribution axes (telemetry migration 0004 on D1). phase defaults to '' --
  -- the unattributed slice, a REAL group in the rollup -- and turn_index is NULLABLE because the
  -- proxy path has no job-scoped counter, where a 0 would read as "the first turn".
