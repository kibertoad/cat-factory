...
import type { MachineTelemetryReadClient, TelemetryReadRequest } from '@cat-factory/server'
...
describe('mothership-mode telemetry read-through', () => {
  ...
  it('serves a run the LOCAL store holds without touching the mothership', async () => {
    ...
    const repos = withTelemetryReadThrough(store, client)
  ...
  it('pages the drain forwards across a shared millisecond', async () => {
    // The page size is 3 for snapshots, so a 7-row run needs three requests — and two of its rows
    // share a createdAt, which a timestamp-only cursor would drop from the next page.
    const snapshots = [
    ...
    // Three pages of 3 (the last short, which is what terminates the drain).
    expect(seen).toHaveLength(3)
    expect(seen[0]!.args[0]).toMatchObject({ executionId: 'exe_far', limit: 3 })
    expect(seen[1]!.args[0]).toMatchObject({ cursor: { createdAt: 40, id: 's5' } })
  })
  ...
  it('reports a drain it had to cap rather than returning a short list as the whole run', async () => {
    ...
    const repos = withTelemetryReadThrough(store, client, recording)
