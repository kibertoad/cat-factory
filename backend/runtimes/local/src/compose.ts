...
// `docker compose <args>` via `execFile` and persist each project's scratch files (the repo's
// compose file + the generated publish override) under a host temp dir the daemon can read.
// This is the only place the Docker-Compose environment backend touches `node:*`, keeping the
// integrations provider runtime-neutral.
  ...
  return {
    async compose(args, options) {
      try {
        ...
        // streams; a missing binary rejects with `.code` = 'ENOENT' (a string). Normalize both
        // into a non-throwing result so the provider can surface the daemon's complaint as
        // `step.environment.lastError` instead of a generic 500.
        const e = err as {
          code?: number | string
          stdout?: string
          stderr?: string
