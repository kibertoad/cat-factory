...
// RPC; this module is the single definition of the test secrets, the machine-token claim shape,
// and the mothership-side env base — so the security-relevant token contract is written once.
//
...
/**
 * Mint a `machine`-audience persistence token, scoped to `accountIds` and signed with `secret`.
 * The claim shape (audience pin + account scope + 1h expiry) is the contract
 * `PersistenceController` verifies, so keeping it in one place stops the two specs from drifting.
 */
