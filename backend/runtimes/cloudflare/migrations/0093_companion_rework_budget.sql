...
-- engine hard-coded (3, in `DEFAULT_COMPANION_MAX_ATTEMPTS`) while every sibling loop reads its
-- ceiling off the task's risk policy: the CI fixer, the iterative requirements review, the Tester
-- quality gate, a judge's bounces, the post-release-health watch. So the loop with the widest reach,
-- and the one an operator watching a run actually sees spend model calls, was the one they could not
-- tune.
--
