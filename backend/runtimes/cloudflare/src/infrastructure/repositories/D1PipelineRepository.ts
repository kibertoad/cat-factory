...
export class D1PipelineRepository implements PipelineRepository {
  ...
  async insert(workspaceId: string, pipeline: Pipeline): Promise<void> {
    await this.db
      .prepare(
        'INSERT INTO pipelines (workspace_id, id, name, agent_kinds, gates, thresholds, enabled, consensus, gating, follow_ups, tester_quality, step_options, labels, archived, builtin, version, public, availability, purpose) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      )
      .bind(
        workspaceId,
        pipeline.id,
  ...
  async update(workspaceId: string, pipeline: Pipeline): Promise<void> {
    ...
    await this.db
      .prepare(
        'UPDATE pipelines SET name = ?, agent_kinds = ?, gates = ?, thresholds = ?, enabled = ?, consensus = ?, gating = ?, follow_ups = ?, tester_quality = ?, step_options = ?, labels = ?, archived = ?, version = ?, public = ?, availability = ?, purpose = ? WHERE workspace_id = ? AND id = ?',
      )
      .bind(
        pipeline.name,
        JSON.stringify(pipeline.agentKinds),
