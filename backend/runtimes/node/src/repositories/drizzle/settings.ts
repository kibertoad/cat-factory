...
export class DrizzleWorkspaceSettingsRepository implements WorkspaceSettingsRepository {
  ...
  async upsert(workspaceId: string, settings: WorkspaceSettings): Promise<void> {
    const values = {
      ...
      done_lane_max_items: settings.doneLaneMaxItems,
      done_lane_retention_days: settings.doneLaneRetentionDays,
    ...
    await this.db
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          done_lane_max_items: values.done_lane_max_items,
          done_lane_retention_days: values.done_lane_retention_days,
