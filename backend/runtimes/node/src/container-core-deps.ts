...
import { nodeHostResolver } from './hostResolver.js'
...
function selectNodeEnvironmentsDeps(config: AppConfig, db: DrizzleDb): Partial<CoreDependencies> {
  ...
  return {
    ...
    // Turning a stated balancer NAME into the addresses to dial. `dns.lookup` is the system view,
    // which is the one the socket above would have used; the Worker resolves over DNS-over-HTTPS
    // because workerd exposes no resolver.
    hostResolver: nodeHostResolver,
