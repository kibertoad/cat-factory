...
// Cross-runtime parity for the agent-search-query observability sink against the Node
...
if (databaseUrl) {
  ...
  defineAgentSearchQuerySuite(
    'node',
    () => createDrizzleRepositories(db, clock).agentSearchQueryRepository,
  )
} else {
  describe.skip('[node] agent search queries (set DATABASE_URL to run)', () => {
