...
export interface AcknowledgeKaizenEntry {
...
export interface AddressPublicRunBugFishingFindingsRequest {
  findingIds: string[]
  pipelineId?: string
}
...
export interface CreatePublicTask {
  ...
  /** Length 1..120. */
  riskPolicyId?: string
  taskType?: 'feature' | 'bug' | 'bug-fishing' | 'document' | 'spike' | 'review' | 'ralph' | 'media' | (string & {})
...
export type DebugInfraLogEntryOperation = 'provision' | 'teardown' | 'teardown-verify' | 'status' | 'remediate' | 'dispatch' | 'release' | 'poll-failure'

/** Every `DebugInfraLogEntryOperation` value, for exhaustive handling and runtime validation. */
export const DEBUG_INFRA_LOG_ENTRY_OPERATION_VALUES = ['provision', 'teardown', 'teardown-verify', 'status', 'remediate', 'dispatch', 'release', 'poll-failure'] as const
...
export type DebugRunFailureKind = 'preflight' | 'dispatch' | 'environment' | 'evicted' | 'harness_shutdown' | 'timeout' | 'agent' | 'job_failed' | 'delegated_failed' | 'rejected' | 'companion_rejected' | 'stalled' | 'state_unreadable' | 'cancelled' | 'unknown'

/** Every `DebugRunFailureKind` value, for exhaustive handling and runtime validation. */
export const DEBUG_RUN_FAILURE_KIND_VALUES = ['preflight', 'dispatch', 'environment', 'evicted', 'harness_shutdown', 'timeout', 'agent', 'job_failed', 'delegated_failed', 'rejected', 'companion_rejected', 'stalled', 'state_unreadable', 'cancelled', 'unknown'] as const
...
export interface DebugRunOverviewLlm {
  ...
  reporting?: DebugRunOverviewLlmReporting
  totals: DebugLlmTotals
}

export interface DebugRunOverviewLlmReporting {
  delegatedStepsWithoutUsage: number
  executors: string[]
}
...
export interface GetPublicRepoFileResponse {
  content: string
  name: string
  owner: string
  path: string
...
export interface GetPublicRunOutcomeResponseEnvironmentsVariant1Entry {
  ...
  detail: string | null
  /** Always present; `null` when the server has no value for it. */
  detailKind: GetPublicRunOutcomeResponseEnvironmentsVariant1EntryDetailKind | null
  /** Always present; `null` when the server has no value for it. */
...
export type GetPublicRunOutcomeResponseEnvironmentsVariant1EntryDetailKind = 'fault' | 'note'

/** Every `GetPublicRunOutcomeResponseEnvironmentsVariant1EntryDetailKind` value, for exhaustive handling and runtime validation. */
export const GET_PUBLIC_RUN_OUTCOME_RESPONSE_ENVIRONMENTS_VARIANT1_ENTRY_DETAIL_KIND_VALUES = ['fault', 'note'] as const
...
export type GetPublicRunOutcomeResponseTestsVariant0Gap = 'run_unavailable' | 'no_tester_step' | 'tester_not_reported' | 'verified_by_committed_tests'

/** Every `GetPublicRunOutcomeResponseTestsVariant0Gap` value, for exhaustive handling and runtime validation. */
export const GET_PUBLIC_RUN_OUTCOME_RESPONSE_TESTS_VARIANT0_GAP_VALUES = ['run_unavailable', 'no_tester_step', 'tester_not_reported', 'verified_by_committed_tests'] as const
...
export interface GetPublicTrackerWritebackResponse {
  /** Always present; `null` when the server has no value for it. */
  updatedAt: number | null
  writeback: GetPublicTrackerWritebackResponseWriteback
}

export interface GetPublicTrackerWritebackResponseWriteback {
  commentOnPrOpen: boolean
  questionsOnPark: boolean
  resolveOnMerge: boolean
}
...
export type InvokePublicUseCaseRequestParametersValue =
  | string
  | string[]
  | boolean
  | number
...
export const INVOKE_PUBLIC_USE_CASE_RESPONSE_FINISH_REASON_VALUES = ['stop', 'length', 'content-filter', 'other'] as const
...
export interface ListPublicEnvironmentConnectionsResponse {
  connections: ListPublicEnvironmentConnectionsResponseConnection[]
}

export interface ListPublicEnvironmentConnectionsResponseConnection {
  /** Always present; `null` when the server has no value for it. */
...
export const LIST_PUBLIC_ENVIRONMENT_MANIFEST_TYPES_RESPONSE_MANIFEST_TYPE_SOURCE_VALUES = ['registered', 'workspace'] as const
...
export type ListPublicKaizenEntriesAcknowledged = 'true' | 'false'

/** Every `ListPublicKaizenEntriesAcknowledged` value, for exhaustive handling and runtime validation. */
export const LIST_PUBLIC_KAIZEN_ENTRIES_ACKNOWLEDGED_VALUES = ['true', 'false'] as const
...
export type ListPublicRiskPoliciesResponsePolicyAutonomy = 'attended' | 'unattended'

/** Every `ListPublicRiskPoliciesResponsePolicyAutonomy` value, for exhaustive handling and runtime validation. */
export const LIST_PUBLIC_RISK_POLICIES_RESPONSE_POLICY_AUTONOMY_VALUES = ['attended', 'unattended'] as const
...
export const LIST_PUBLIC_USE_CASES_RESPONSE_USE_CASE_MODEL_UNAVAILABLE_REASON_VALUES = ['provider_unavailable', 'container_only'] as const
...
export const LIST_PUBLIC_USE_CASES_RESPONSE_USE_CASE_PARAMETER_TYPE_VALUES = ['text', 'textarea', 'number', 'select', 'checkbox', 'checkbox-group'] as const
...
export type NotificationType = 'merge_review' | 'pipeline_complete' | 'ci_failed' | 'deploy_blocked' | 'test_failed' | 'requirement_review' | 'clarity_review' | 'release_regression' | 'decision_required' | 'human_test_ready' | 'visual_confirmation_ready' | 'human_review' | 'followup_pending' | 'fork_decision_pending' | 'judge_review' | 'pr_review_ready' | 'bug_fishing_triage' | 'initiative' | 'platform_health' | 'infra_unreachable' | 'budget_paused' | 'budget_threshold' | 'key_drift' | 'merge_tag_request'

/** Every `NotificationType` value, for exhaustive handling and runtime validation. */
export const NOTIFICATION_TYPE_VALUES = ['merge_review', 'pipeline_complete', 'ci_failed', 'deploy_blocked', 'test_failed', 'requirement_review', 'clarity_review', 'release_regression', 'decision_required', 'human_test_ready', 'visual_confirmation_ready', 'human_review', 'followup_pending', 'fork_decision_pending', 'judge_review', 'pr_review_ready', 'bug_fishing_triage', 'initiative', 'platform_health', 'infra_unreachable', 'budget_paused', 'budget_threshold', 'key_drift', 'merge_tag_request'] as const
...
export interface PrReportEnvironmentsEntry {
  error?: string | null
  frameId: string
  remediation?: PrReportEnvironmentsEntryRemediation | null
  status: PrReportEnvironmentsEntryStatus
  url?: string | null
}

export interface PrReportEnvironmentsEntryRemediation {
  deployFix?: PrReportEnvironmentsEntryRemediationDeployFix | null
  investigation?: PrReportEnvironmentsEntryRemediationInvestigation | null
}
...
export interface PrReportEnvironmentsEntryRemediationInvestigation {
  ...
  maxAttempts?: number | null
  ranActions: string[]
  waitExtensions: number
  withheld?: string | null
}

export type PrReportEnvironmentsEntryRemediationInvestigationFaultLayer = 'provider' | 'platform' | 'deployment' | 'unknown'
...
export const PUBLIC_ANSWER_FOLLOW_UP_RESOLUTION_VALUES = ['answered', 'closed'] as const
...
export interface PublicApprovalGateDecision {
  approvalId: string
  blockingFindings: PublicApprovalGateDecisionBlockingFinding[]
  exceeded: boolean
  /** Always present; `null` when the server has no value for it. */
...
export interface PublicApprovalGateDecisionBlockingFinding {
  /** Always present; `null` when the server has no value for it. */
...
export interface PublicBugFishingDecision {
  ...
  findings: PublicBugFishingFinding[]
  kind: 'bug-fishing'
  /** Always present; `null` when the server has no value for it. */
  model: string | null
  phases: PublicBugFishingPhase[]
  /** Always present; `null` when the server has no value for it. */
  plan: PublicBugFishingPlan | null
  status: PublicBugFishingDecisionStatus
  stepIndex: number
  stepKind: string
}

export type PublicBugFishingDecisionStatus = 'fishing' | 'awaiting_triage' | 'done'

/** Every `PublicBugFishingDecisionStatus` value, for exhaustive handling and runtime validation. */
...
export interface PublicBugFishingFinding {
  ...
  severity: Severity
  /** Always present; `null` when the server has no value for it. */
  spawn: PublicBugFishingSpawn | null
  /** Always present; `null` when the server has no value for it. */
...
export type PublicBugFishingFindingKind = 'bug' | 'logic-gap' | 'edge-case' | 'footgun' | 'requirement-gap' | 'other'

/** Every `PublicBugFishingFindingKind` value, for exhaustive handling and runtime validation. */
export const PUBLIC_BUG_FISHING_FINDING_KIND_VALUES = ['bug', 'logic-gap', 'edge-case', 'footgun', 'requirement-gap', 'other'] as const

export interface PublicBugFishingPhase {
  /** Always present; `null` when the server has no value for it. */
...
export type PublicBugFishingPhaseStatus = 'pending' | 'fishing' | 'completed' | 'failed'

/** Every `PublicBugFishingPhaseStatus` value, for exhaustive handling and runtime validation. */
export const PUBLIC_BUG_FISHING_PHASE_STATUS_VALUES = ['pending', 'fishing', 'completed', 'failed'] as const

export interface PublicBugFishingPlan {
  passBudget: number
  plannedCells: number
  /** Always present; `null` when the server has no value for it. */
  surveyUnavailableReason: string | null
  treeTruncated: boolean
  unfished: PublicBugFishingUnfishedCell[]
}

export interface PublicBugFishingSpawn {
  /** Always present; `null` when the server has no value for it. */
  executionId: string | null
  /** Always present; `null` when the server has no value for it. */
  failureReason: string | null
...
export type PublicBugFishingSpawnStatus = 'pending' | 'spawned' | 'failed'

/** Every `PublicBugFishingSpawnStatus` value, for exhaustive handling and runtime validation. */
export const PUBLIC_BUG_FISHING_SPAWN_STATUS_VALUES = ['pending', 'spawned', 'failed'] as const

export interface PublicBugFishingUnfishedCell {
  phaseId: string
  phaseTitle: string
  territoryId: string
...
export interface PublicKaizenEntry {
  acknowledged: boolean
  /** Always present; `null` when the server has no value for it. */
  ...
  agentKind: string
  /** Always present; `null` when the server has no value for it. */
  combo: PublicKaizenEntryCombo | null
  comboKey: string
  createdAt: number
  entryId: string
  /** Always present; `null` when the server has no value for it. */
  ...
  summary: string
  /** Always present; `null` when the server has no value for it. */
  task: PublicKaizenEntryTask | null
  taskId: string
  updatedAt: number
}

export interface PublicKaizenEntryCombo {
...
export interface PublicKaizenEntryList {
  entries: PublicKaizenEntry[]
  /** Always present; `null` when the server has no value for it. */
  nextCursor: string | null
}

export type PublicKaizenEntryStatus = 'scheduled' | 'running' | 'complete' | 'failed'

/** Every `PublicKaizenEntryStatus` value, for exhaustive handling and runtime validation. */
export const PUBLIC_KAIZEN_ENTRY_STATUS_VALUES = ['scheduled', 'running', 'complete', 'failed'] as const

export interface PublicKaizenEntryTask {
  /** Always present; `null` when the server has no value for it. */
  serviceId: string | null
  /** Always present; `null` when the server has no value for it. */
...
export interface PublicPromptFragment {
  appliesTo?: PublicPromptFragmentAppliesTo
  category: string
  fragmentId: string
  summary: string
  tags: string[]
...
export interface PublicPromptFragmentAppliesTo {
  agentKinds?: string[]
  blockTypes?: PublicServiceType[]
}
...
/** Every `PublicPromptFragmentTier` value, for exhaustive handling and runtime validation. */
export const PUBLIC_PROMPT_FRAGMENT_TIER_VALUES = ['builtin', 'account', 'workspace'] as const
...
export interface PublicPrReviewDecisionPostReport {
  ...
  failures: PublicPrReviewDecisionPostReportFailure[]
  folded: number
  posted: number
}

export interface PublicPrReviewDecisionPostReportFailure {
  findingId: string
  /** Always present; `null` when the server has no value for it. */
  line: number | null
  path: string
...
export type PublicRunArtifactKind = 'screenshot' | 'reference' | 'asset'

/** Every `PublicRunArtifactKind` value, for exhaustive handling and runtime validation. */
export const PUBLIC_RUN_ARTIFACT_KIND_VALUES = ['screenshot', 'reference', 'asset'] as const
...
export interface PublicTask {
  ...
  status: TaskStatus
  taskId: string
  taskType: 'feature' | 'bug' | 'bug-fishing' | 'document' | 'spike' | 'review' | 'ralph' | 'media' | 'recurring' | (string & {})
  title: string
}
...
export type StartPublicRepoBootstrapResponseDelivery = 'pull_request' | 'direct_push'

/** Every `StartPublicRepoBootstrapResponseDelivery` value, for exhaustive handling and runtime validation. */
export const START_PUBLIC_REPO_BOOTSTRAP_RESPONSE_DELIVERY_VALUES = ['pull_request', 'direct_push'] as const

export type StartPublicRepoBootstrapResponseStatus = 'pending' | 'running' | 'awaiting_review' | 'succeeded' | 'failed'

/** Every `StartPublicRepoBootstrapResponseStatus` value, for exhaustive handling and runtime validation. */
export const START_PUBLIC_REPO_BOOTSTRAP_RESPONSE_STATUS_VALUES = ['pending', 'running', 'awaiting_review', 'succeeded', 'failed'] as const
...
export interface UpdatePublicTrackerWritebackRequest {
  writeback?: UpdatePublicTrackerWritebackRequestWriteback
}

export interface UpdatePublicTrackerWritebackRequestWriteback {
  commentOnPrOpen?: boolean
  questionsOnPark?: boolean
  resolveOnMerge?: boolean
}
