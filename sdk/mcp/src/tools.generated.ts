...
// The tool table: one entry per `/api/v1` operation the facade exposes, surface version 1.74.0.
//
...
const QUERY_REPOS_GET_FILE: readonly QueryParam[] = [
  { name: 'path', required: true },
  { name: 'ref', required: false },
]
const QUERY_REPOS_LIST_AVAILABLE: readonly QueryParam[] = [
...
const QUERY_KAIZEN_LIST_ENTRIES: readonly QueryParam[] = [
  { name: 'limit', required: false },
  { name: 'cursor', required: false },
...
export const CAT_FACTORY_TOOLS: readonly CatFactoryTool[] = [
  ...
  },
  {
    ...
    invoke: (client, args) => client.services.delete(str(args, 'serviceId')),
  },
  {
    name: 'services_list',
    ...
    outputSchema: {"type":"object","properties":{"services":{"type":"array","items":{"type":"object","properties":{"description":{"type":"string"},"provisioning":{"description":"Discriminated by `type` (kubernetes | infraless | custom). A newer deployment may report a variant not in this list."},"serviceId":{"type":"string"},"status":{"type":"string","description":"One of: planned, ready, in_progress, blocked, pr_ready, done. A newer deployment may report a member not in this list."},"title":{"type":"string"},"type":{"type":"string","description":"One of: frontend, service, library, document, api, database, queue, integration, external, environment. A newer deployment may report a member not in this list."}}}}}},
    invoke: (client, args) => client.services.list(),
  },
  {
    name: 'services_update',
    title: 'Patch a service, including where its per-run manifests live',
    group: 'services',
    operationId: 'updatePublicService',
    readOnly: false,
    description: 'Patch a service, including where its per-run manifests live\n\nChange a service’s authored fields, and declare its `provisioning`: where the manifests for a per-run environment are read from. That second half is what a connected cluster alone cannot supply, because the platform keeps “which cluster” (one per workspace) apart from “which manifests” (one set per service). An omitted `provisioning` leaves the stored one alone rather than clearing it, so correcting a title cannot un-deploy a service; send `provisioning: { "type": "infraless" }` to take the pin BACK, which leaves the service with no environment to provision and reads back with no `provisioning` at all. Board coordinates are deliberately absent, as they are on service creation.\n\nCalls `PATCH /api/v1/services/{serviceId}` (operation `updatePublicService`).',
    inputSchema: {"type":"object","properties":{"serviceId":{"type":"string","description":"Path parameter `serviceId`."},"body":{"type":"object","properties":{"description":{"type":"string","maxLength":2000},"provisioning":{"anyOf":[{"type":"object","properties":{"manifestSource":{"anyOf":[{"type":"object","properties":{"path":{"type":"string","minLength":1,"maxLength":500},"renderer":{"type":"string","enum":["raw","kustomize"]},"type":{"const":"colocated"}},"required":["path","type"]},{"type":"object","properties":{"path":{"type":"string","minLength":1,"maxLength":500},"ref":{"type":"string","minLength":1},"renderer":{"type":"string","enum":["raw","kustomize"]},"repo":{"type":"string"},"type":{"const":"separate"}},"required":["path","repo","type"]}],"description":"Discriminated by `type` (colocated | separate)."},"type":{"const":"kubernetes"}},"required":["manifestSource","type"]},{"type":"object","properties":{"type":{"const":"infraless"}},"required":["type"]},{"type":"object","properties":{"manifestId":{"type":"string","minLength":1,"maxLength":64},"manifestPath":{"type":"string","maxLength":500},"type":{"const":"custom"}},"required":["manifestId","type"]}],"description":"Discriminated by `type` (kubernetes | infraless | custom)."},"title":{"type":"string","minLength":1,"maxLength":200}},"description":"The request body."}},"additionalProperties":false,"required":["serviceId"]},
    outputSchema: {"type":"object","properties":{"description":{"type":"string"},"provisioning":{"description":"Discriminated by `type` (kubernetes | infraless | custom). A newer deployment may report a variant not in this list."},"serviceId":{"type":"string"},"status":{"type":"string","description":"One of: planned, ready, in_progress, blocked, pr_ready, done. A newer deployment may report a member not in this list."},"title":{"type":"string"},"type":{"type":"string","description":"One of: frontend, service, library, document, api, database, queue, integration, external, environment. A newer deployment may report a member not in this list."}}},
    invoke: (client, args) => client.services.update(str(args, 'serviceId'), args.body as Parameters<CatFactoryClient['services']['update']>[1]),
  },
  {
    name: 'spec_get',
  ...
  },
  {
    ...
    outputSchema: {"type":"object","properties":{"anchor":{"type":"string","description":"One of: present, absent, unparsed, not_read. A newer deployment may report a member not in this list."},"features":{"type":"array","items":{"type":"object","properties":{"chars":{"type":"number"},"content":{"type":"string"},"group":{"type":"string"},"module":{"type":"string"},"path":{"type":"string"},"totalChars":{"type":"number"},"truncated":{"type":"boolean"}}}},"issues":{"type":"array","items":{"type":"object","properties":{"dropped":{"anyOf":[{"type":"number"},{"type":"null"}]},"kind":{"type":"string","description":"One of: read_failed, unparsed, partial, unread. A newer deployment may report a member not in this list."},"path":{"type":"string"}}}},"provenance":{"anyOf":[{"type":"object","properties":{"commit":{"anyOf":[{"type":"string"},{"type":"null"}]},"owner":{"type":"string"},"provider":{"type":"string","description":"One of: github, gitlab. A newer deployment may report a member not in this list."},"ref":{"type":"string"},"repo":{"type":"string"}}},{"type":"null"}]},"runId":{"type":"string"},"spec":{"anyOf":[{"type":"object","properties":{"modules":{"type":"array","items":{"type":"object","properties":{"groups":{"type":"array","items":{"type":"object","properties":{"name":{"type":"string"},"requirements":{"type":"array","items":{"type":"object","properties":{"acceptance":{"type":"array","items":{"type":"object","properties":{"given":{"type":"string"},"id":{"type":"string"},"outcome":{"type":"string"},"when":{"type":"string"}}}},"id":{"type":"string"},"kind":{"type":"string","description":"One of: functional, nonfunctional, constraint. A newer deployment may report a member not in this list."},"priority":{"type":"string","description":"One of: must, should, could. A newer deployment may report a member not in this list."},"sourceBlockIds":{"type":"array","items":{"type":"string"}},"state":{"type":"string","description":"One of: aspirational, established. A newer deployment may report a member not in this list."},"statement":{"type":"string"},"title":{"type":"string"}}}},"rules":{"type":"array","items":{"type":"object","properties":{"id":{"type":"string"},"rationale":{"type":"string"},"rule":{"type":"string"},"sourceBlockIds":{"type":"array","items":{"type":"string"}}}}},"summary":{"type":"string"}}}},"name":{"type":"string"},"summary":{"type":"string"}}}},"service":{"type":"string"},"summary":{"type":"string"}}},{"type":"null"}]},"truncations":{"type":"array","items":{"type":"object","properties":{"section":{"type":"string","description":"One of: requirements, rules, acceptance, features, issues. A newer deployment may report a member not in this list."},"shown":{"type":"number"},"total":{"type":"number"}}}}}},
    invoke: (client, args) => client.spec.getForRun(str(args, 'runId')),
  },
  {
    name: 'repos_bootstrap',
    title: 'Create a repository and adapt it with the bootstrapper agent',
    group: 'repos',
    ...
    invoke: (client, args) => client.repos.bootstrap(args.body as Parameters<CatFactoryClient['repos']['bootstrap']>[0]),
  },
  {
    name: 'repos_get_bootstrap',
    title: 'Poll one repository bootstrap',
    group: 'repos',
  ...
  },
  {
    ...
    inputSchema: {"type":"object","properties":{"taskId":{"type":"string","description":"Path parameter `taskId`."}},"additionalProperties":false,"required":["taskId"]},
    outputSchema: {"type":"object","properties":{"createdAt":{"type":"number"},"currentStep":{"type":"number"},"error":{"anyOf":[{"type":"object","properties":{"code":{"type":"string"},"message":{"type":"string"}}},{"type":"null"}]},"externalIdentity":{"anyOf":[{"type":"string"},{"type":"null"}]},"externalIdentityWithheld":{"type":"boolean"},"pullRequest":{"anyOf":[{"type":"object","properties":{"branch":{"anyOf":[{"type":"string"},{"type":"null"}]},"url":{"type":"string"}}},{"type":"null"}]},"runId":{"type":"string"},"status":{"type":"string","description":"One of: running, blocked, paused, done, failed. A newer deployment may report a member not in this list."},"steps":{"type":"array","items":{"type":"object","properties":{"agentKind":{"type":"string"},"data":{"anyOf":[{},{"type":"null"}]},"output":{"anyOf":[{"type":"string"},{"type":"null"}]},"progress":{"type":"number"},"skipped":{"type":"boolean"},"state":{"type":"string","description":"One of: pending, working, waiting_decision, done. A newer deployment may report a member not in this list."},"subtasks":{"anyOf":[{"type":"object","properties":{"completed":{"type":"number"},"inProgress":{"type":"number"},"total":{"type":"number"}}},{"type":"null"}]},"truncated":{"type":"boolean"}}}},"taskId":{"type":"string"}}},
  ...
  },
  {
    ...
    title: 'Connect the workspace to the cluster its environments deploy onto',
    group: 'environments',
    operationId: 'connectPublicEnvironment',
    readOnly: false,
    description: 'Connect the workspace to the cluster its environments deploy onto\n\nBind environment provisioning to a Kubernetes cluster: the apiserver, how its TLS is verified, the namespace template, and how an environment URL is derived once manifests are applied. The secret bundle authenticating the connection is write-only; the response reports which secret KEYS were stored and never their values. Idempotent, so re-connecting replaces rather than accumulating.\n\nCalls `POST /api/v1/environments/connections` (operation `connectPublicEnvironment`).',
    inputSchema: {"type":"object","properties":{"body":{"type":"object","properties":{"connection":{"anyOf":[{"type":"object","properties":{"engine":{"const":"kubernetes"},"kubernetes":{"type":"object","properties":{"annotations":{"type":"object","additionalProperties":{"type":"string"}},"apiServerUrl":{"type":"string","maxLength":500},"caCertPem":{"type":"string"},"defaultTtlMs":{"type":"number","minimum":60000},"imageTemplate":{"type":"string","maxLength":500},"insecureSkipTlsVerify":{"type":"boolean"},"label":{"type":"string","minLength":1,"maxLength":120},"labels":{"type":"object","additionalProperties":{"type":"string"}},"namespaceTemplate":{"type":"string","minLength":1,"maxLength":200},"url":{"anyOf":[{"type":"object","properties":{"hostTemplate":{"type":"string","minLength":1,"maxLength":500},"port":{"type":"integer","minimum":1,"maximum":65535},"scheme":{"type":"string","enum":["http","https"]},"source":{"const":"ingressTemplate"}},"required":["hostTemplate","source"]},{"type":"object","properties":{"ingressName":{"type":"string","minLength":1},"scheme":{"type":"string","enum":["http","https"]},"source":{"const":"ingressStatus"}},"required":["source"]},{"type":"object","properties":{"port":{"type":"integer","minimum":1,"maximum":65535},"scheme":{"type":"string","enum":["http","https"]},"serviceName":{"type":"string","minLength":1},"source":{"const":"serviceStatus"}},"required":["serviceName","source"]},{"type":"object","properties":{"gatewayName":{"type":"string","minLength":1},"scheme":{"type":"string","enum":["http","https"]},"source":{"const":"gatewayStatus"}},"required":["source"]},{"type":"object","properties":{"httpRouteName":{"type":"string","minLength":1},"scheme":{"type":"string","enum":["http","https"]},"source":{"const":"httpRouteStatus"}},"required":["source"]}],"description":"Discriminated by `source` (ingressTemplate | ingressStatus | serviceStatus | gatewayStatus | httpRouteStatus)."}},"required":["apiServerUrl","label","url"]}},"required":["engine","kubernetes"]}],"description":"Discriminated by `engine` (kubernetes)."},"secrets":{"type":"object","additionalProperties":{"type":"string"}}},"required":["connection","secrets"],"description":"The request body."}},"additionalProperties":false,"required":["body"]},
    outputSchema: {"type":"object","properties":{"apiServerUrl":{"type":"string"},"engine":{"type":"string"},"label":{"type":"string"},"provisionType":{"type":"string"},"secretKeys":{"type":"array","items":{"type":"string"}}}},
    invoke: (client, args) => client.environments.connect(args.body as Parameters<CatFactoryClient['environments']['connect']>[0]),
  },
  {
    name: 'environments_list_connections',
    title: 'List the environment connections this workspace holds',
    group: 'environments',
    operationId: 'listPublicEnvironmentConnections',
    readOnly: true,
    description: 'List the environment connections this workspace holds\n\nEvery registered environment handler, with the provision type it serves, the engine and backend kind behind it, its endpoint and the secret KEYS it holds, never their values. The read half of the connect call, and the half that was missing: a deployment that registers its handlers programmatically (the documented path for a multi-tenant deployment) had no way for a headless caller to confirm the registration landed, so “the backend accepts our credential” and “this workspace has a handler for that backend” collapsed into one unanswerable question. It reports every engine, including a handler for an environment backend the deployment registered in code, so `engine` and `backendKind` are open strings rather than a fixed set.\n\nCalls `GET /api/v1/environments/connections` (operation `listPublicEnvironmentConnections`).',
    inputSchema: {"type":"object","properties":{},"additionalProperties":false},
    outputSchema: {"type":"object","properties":{"connections":{"type":"array","items":{"type":"object","properties":{"acceptsManifestId":{"anyOf":[{"type":"string"},{"type":"null"}]},"backendKind":{"type":"string"},"connectedAt":{"type":"number"},"endpoint":{"type":"string"},"engine":{"type":"string"},"label":{"type":"string"},"manifestId":{"anyOf":[{"type":"string"},{"type":"null"}]},"provisionType":{"type":"string"},"secretKeys":{"type":"array","items":{"type":"string"}}}}}}},
    invoke: (client, args) => client.environments.listConnections(),
  },
  {
    name: 'environments_list_manifest_types',
    title: 'List the custom manifest types a service can pin',
    group: 'environments',
    operationId: 'listPublicEnvironmentManifestTypes',
    readOnly: true,
    description: 'List the custom manifest types a service can pin\n\nEvery custom-manifest-type id a service’s `custom` provisioning may name, with the label and default manifest path of each, and whether the deployment registered it in code (`registered`) or the workspace defined it (`workspace`). Those two are fixed by different people, which is why the source is reported. The read exists because a pin is checked against no registry on the way in: an id no handler serves is accepted and fails at the `deployer` step of a run already paid for, so a caller lists first and refuses before it spends.\n\nCalls `GET /api/v1/environments/manifest-types` (operation `listPublicEnvironmentManifestTypes`).',
    inputSchema: {"type":"object","properties":{},"additionalProperties":false},
    outputSchema: {"type":"object","properties":{"manifestTypes":{"type":"array","items":{"type":"object","properties":{"defaultManifestPath":{"anyOf":[{"type":"string"},{"type":"null"}]},"label":{"type":"string"},"manifestId":{"type":"string"},"source":{"type":"string","description":"One of: registered, workspace. A newer deployment may report a member not in this list."}}}}}},
    invoke: (client, args) => client.environments.listManifestTypes(),
  },
  {
    name: 'environments_test_connection',
    title: 'Probe a candidate cluster connection without saving it',
    group: 'environments',
    operationId: 'testPublicEnvironmentConnection',
    readOnly: false,
    description: 'Probe a candidate cluster connection without saving it\n\nReach the apiserver with the supplied credentials and report what came back, persisting nothing. Worth a call of its own because the alternative is discovering an unreachable cluster or an expired token on the deploy step of a run that has already paid for a design pass and an implementation. A cluster that refuses the credential is an ANSWER, so it is a 200 carrying `ok: false` rather than an error.\n\nCalls `POST /api/v1/environments/connections/test` (operation `testPublicEnvironmentConnection`).',
    inputSchema: {"type":"object","properties":{"body":{"type":"object","properties":{"connection":{"anyOf":[{"type":"object","properties":{"engine":{"const":"kubernetes"},"kubernetes":{"type":"object","properties":{"annotations":{"type":"object","additionalProperties":{"type":"string"}},"apiServerUrl":{"type":"string","maxLength":500},"caCertPem":{"type":"string"},"defaultTtlMs":{"type":"number","minimum":60000},"imageTemplate":{"type":"string","maxLength":500},"insecureSkipTlsVerify":{"type":"boolean"},"label":{"type":"string","minLength":1,"maxLength":120},"labels":{"type":"object","additionalProperties":{"type":"string"}},"namespaceTemplate":{"type":"string","minLength":1,"maxLength":200},"url":{"anyOf":[{"type":"object","properties":{"hostTemplate":{"type":"string","minLength":1,"maxLength":500},"port":{"type":"integer","minimum":1,"maximum":65535},"scheme":{"type":"string","enum":["http","https"]},"source":{"const":"ingressTemplate"}},"required":["hostTemplate","source"]},{"type":"object","properties":{"ingressName":{"type":"string","minLength":1},"scheme":{"type":"string","enum":["http","https"]},"source":{"const":"ingressStatus"}},"required":["source"]},{"type":"object","properties":{"port":{"type":"integer","minimum":1,"maximum":65535},"scheme":{"type":"string","enum":["http","https"]},"serviceName":{"type":"string","minLength":1},"source":{"const":"serviceStatus"}},"required":["serviceName","source"]},{"type":"object","properties":{"gatewayName":{"type":"string","minLength":1},"scheme":{"type":"string","enum":["http","https"]},"source":{"const":"gatewayStatus"}},"required":["source"]},{"type":"object","properties":{"httpRouteName":{"type":"string","minLength":1},"scheme":{"type":"string","enum":["http","https"]},"source":{"const":"httpRouteStatus"}},"required":["source"]}],"description":"Discriminated by `source` (ingressTemplate | ingressStatus | serviceStatus | gatewayStatus | httpRouteStatus)."}},"required":["apiServerUrl","label","url"]}},"required":["engine","kubernetes"]}],"description":"Discriminated by `engine` (kubernetes)."},"secrets":{"type":"object","additionalProperties":{"type":"string"}}},"required":["connection"],"description":"The request body."}},"additionalProperties":false,"required":["body"]},
    outputSchema: {"type":"object","properties":{"message":{"anyOf":[{"type":"string"},{"type":"null"}]},"ok":{"type":"boolean"}}},
    invoke: (client, args) => client.environments.testConnection(args.body as Parameters<CatFactoryClient['environments']['testConnection']>[0]),
  },
  {
    name: 'models_list',
    title: 'List the models a run in this workspace could dispatch to',
    group: 'models',
    ...
    inputSchema: {"type":"object","properties":{},"additionalProperties":false},
    outputSchema: {"type":"object","properties":{"excludesUserScopedModels":{"type":"boolean"},"models":{"type":"array","items":{"type":"object","properties":{"available":{"type":"boolean"},"label":{"type":"string"},"modelId":{"type":"string"},"personalSubscription":{"type":"boolean"},"policyBlocked":{"type":"boolean"},"provider":{"type":"string"},"subscriptionConfigured":{"anyOf":[{"type":"boolean"},{"type":"null"}]},"userScoped":{"type":"boolean"}}}}}},
    invoke: (client, args) => client.models.list(),
  },
  {
    name: 'vcs_get_connection',
    title: 'Read the workspace’s source-control connection and what it may do',
    group: 'vcs',
    operationId: 'getPublicVcsConnection',
    readOnly: true,
    description: 'Read the workspace’s source-control connection and what it may do\n\nThe connected account, how the workspace authenticates to it, and the two permissions that decide whether an automated flow can complete: whether the platform may create repositories, and whether it may write workflow files. Both are enforced by the provider at push time, so a caller that cannot read them discovers a missing workflow permission as a repository that bootstrapped and then failed to gain its CI workflow. Provider-neutral: a GitLab-connected workspace answers here too. `connection` is null when nothing is connected, which is a state rather than an error.\n\nCalls `GET /api/v1/vcs/connection` (operation `getPublicVcsConnection`).',
    inputSchema: {"type":"object","properties":{},"additionalProperties":false},
    outputSchema: {"type":"object","properties":{"connection":{"anyOf":[{"type":"object","properties":{"accountLogin":{"type":"string"},"canCreateRepos":{"type":"boolean"},"canManageWorkflows":{"type":"boolean"},"method":{"type":"string","description":"One of: app, pat. A newer deployment may report a member not in this list."},"provider":{"type":"string","description":"One of: github, gitlab. A newer deployment may report a member not in this list."}}},{"type":"null"}]}}},
    invoke: (client, args) => client.vcs.getConnection(),
  },
  {
    name: 'tracker_get_writeback',
    title: 'Read the workspace’s tracker writeback disposition',
    group: 'tracker',
    operationId: 'getPublicTrackerWriteback',
    readOnly: true,
    description: 'Read the workspace’s tracker writeback disposition\n\nWhat this workspace does to a task’s LINKED tracker issue as its pull request progresses: comment when the pull request opens, comment and close the issue when it merges, and post a headless run’s parked requirements-review findings so the reporter can answer where they filed. Worth reading before filing a ticket-linked task, since it decides whether the issue the work came from ever hears the outcome. `updatedAt` is null when nobody has chosen a disposition, in which case the values are this deployment’s defaults (all three ON). Requires an `admin` key.\n\nCalls `GET /api/v1/tracker/writeback` (operation `getPublicTrackerWriteback`).',
    inputSchema: {"type":"object","properties":{},"additionalProperties":false},
    outputSchema: {"type":"object","properties":{"updatedAt":{"anyOf":[{"type":"number"},{"type":"null"}]},"writeback":{"type":"object","properties":{"commentOnPrOpen":{"type":"boolean"},"questionsOnPark":{"type":"boolean"},"resolveOnMerge":{"type":"boolean"}}}}},
    invoke: (client, args) => client.tracker.getWriteback(),
  },
  {
    name: 'tracker_update_writeback',
    title: 'Change the workspace’s tracker writeback disposition',
    group: 'tracker',
    operationId: 'updatePublicTrackerWriteback',
    readOnly: false,
    description: 'Change the workspace’s tracker writeback disposition\n\nTurn one or more writeback actions on or off. A MERGE: an action you omit keeps its stored value, so a caller acting on one decision cannot silently move the other two. This is workspace-wide configuration, so it changes what happens to every task’s ticket on the board; the read beside it reports `updatedAt` so a caller can see whether it is about to overwrite somebody’s choice. An empty patch is a no-op and does not stamp `updatedAt`. Requires an `admin` key.\n\nCalls `PATCH /api/v1/tracker/writeback` (operation `updatePublicTrackerWriteback`).',
    inputSchema: {"type":"object","properties":{"body":{"type":"object","properties":{"writeback":{"type":"object","properties":{"commentOnPrOpen":{"type":"boolean"},"questionsOnPark":{"type":"boolean"},"resolveOnMerge":{"type":"boolean"}}}},"description":"The request body."}},"additionalProperties":false},
    outputSchema: {"type":"object","properties":{"updatedAt":{"anyOf":[{"type":"number"},{"type":"null"}]},"writeback":{"type":"object","properties":{"commentOnPrOpen":{"type":"boolean"},"questionsOnPark":{"type":"boolean"},"resolveOnMerge":{"type":"boolean"}}}}},
    invoke: (client, args) => client.tracker.updateWriteback(args.body as Parameters<CatFactoryClient['tracker']['updateWriteback']>[0]),
  },
  {
    name: 'risk_policies_list',
    title: 'List the workspace’s risk policies',
    group: 'risk_policies',
    operationId: 'listPublicRiskPolicies',
    readOnly: true,
    description: 'List the workspace’s risk policies\n\nThe policy library, including which row is the workspace default that a task pinning none resolves. `autoMergeEnabled` is the master switch that decides whether a run can land its pull request without a person; `dryRunRoles` names the roles whose runs the policy forces into dry-run mode, which is the difference between “this policy merges” and “this policy merges for everyone except one role”. A policy also caps CI-fixer attempts, requirement and tester iteration rounds and the release-health watch, which is why it is not called a merge preset; the id is what a task pins as `riskPolicyId`.\n\nCalls `GET /api/v1/risk-policies` (operation `listPublicRiskPolicies`).',
    inputSchema: {"type":"object","properties":{},"additionalProperties":false},
    outputSchema: {"type":"object","properties":{"policies":{"type":"array","items":{"type":"object","properties":{"autoMergeEnabled":{"type":"boolean"},"autonomy":{"type":"string","description":"One of: attended, unattended. A newer deployment may report a member not in this list."},"ciMaxAttempts":{"type":"number"},"dryRunRoles":{"type":"array","items":{"type":"string","description":"One of: admin, member, viewer. A newer deployment may report a member not in this list."}},"isDefault":{"type":"boolean"},"isUnattendedDefault":{"type":"boolean"},"name":{"type":"string"},"policyId":{"type":"string"},"submissionRestrictedRoles":{"type":"array","items":{"type":"string","description":"One of: admin, member, viewer. A newer deployment may report a member not in this list."}}}}}}},
    invoke: (client, args) => client.riskPolicies.list(),
  },
  {
    name: 'model_presets_list',
    title: 'List the workspace’s model presets',
    group: 'model_presets',
    operationId: 'listPublicModelPresets',
    readOnly: true,
    description: 'List the workspace’s model presets\n\nThe preset library, including which row is the workspace default that a task pinning none resolves. `baseModelId` is the model every agent step runs on under the preset, and `overrides` names the agent kinds that run on something else, which is usually the one that matters: two presets often differ only in what the CODER gets. Whether a preset can actually be dispatched to is NOT repeated here, because the models endpoint already answers it while keeping unconfigured apart from refused-by-policy; join on `baseModelId`.\n\nCalls `GET /api/v1/model-presets` (operation `listPublicModelPresets`).',
    inputSchema: {"type":"object","properties":{},"additionalProperties":false},
    outputSchema: {"type":"object","properties":{"presets":{"type":"array","items":{"type":"object","properties":{"baseModelId":{"type":"string"},"isDefault":{"type":"boolean"},"name":{"type":"string"},"overrides":{"type":"object","additionalProperties":{"type":"string"}},"presetId":{"type":"string"}}}}}},
    invoke: (client, args) => client.modelPresets.list(),
  },
  {
    name: 'webhook_delete',
  ...
  },
  {
    ...
    inputSchema: {"type":"object","properties":{},"additionalProperties":false},
    outputSchema: {"type":"object","properties":{"webhook":{"anyOf":[{"type":"object","properties":{"alertEvents":{"type":"array","items":{"type":"string","description":"One of: platform_health.firing, platform_health.resolved. A newer deployment may report a member not in this list."}},"enabled":{"type":"boolean"},"hasSecret":{"type":"boolean"},"id":{"type":"string"},"name":{"type":"string"},"runEvents":{"type":"array","items":{"type":"string","description":"One of: run.started, run.step_completed, run.completed, run.failed. A newer deployment may report a member not in this list."}},"types":{"type":"array","items":{"type":"string","description":"One of: merge_review, pipeline_complete, ci_failed, deploy_blocked, test_failed, requirement_review, clarity_review, release_regression, decision_required, human_test_ready, visual_confirmation_ready, human_review, followup_pending, fork_decision_pending, judge_review, pr_review_ready, bug_fishing_triage, initiative, platform_health, infra_unreachable, budget_paused, budget_threshold, key_drift, merge_tag_request. A newer deployment may report a member not in this list."}},"updatedAt":{"type":"number"},"url":{"type":"string"}}},{"type":"null"}]}}},
  ...
  },
  {
    ...
    inputSchema: {"type":"object","properties":{"body":{"type":"object","properties":{"alertEvents":{"type":"array","items":{"type":"string","enum":["platform_health.firing","platform_health.resolved"]}},"enabled":{"type":"boolean"},"name":{"type":"string","minLength":1,"maxLength":100},"runEvents":{"type":"array","items":{"type":"string","enum":["run.started","run.step_completed","run.completed","run.failed"]}},"secret":{"type":"string","minLength":16,"maxLength":200},"types":{"type":"array","items":{"type":"string","enum":["merge_review","pipeline_complete","ci_failed","deploy_blocked","test_failed","requirement_review","clarity_review","release_regression","decision_required","human_test_ready","visual_confirmation_ready","human_review","followup_pending","fork_decision_pending","judge_review","pr_review_ready","bug_fishing_triage","initiative","platform_health","infra_unreachable","budget_paused","budget_threshold","key_drift","merge_tag_request"]}},"url":{"type":"string","maxLength":2000}},"description":"The request body."}},"additionalProperties":false},
  ...
  },
  {
    ...
    outputSchema: {"type":"object","properties":{"decisions":{"type":"array","items":{"description":"Discriminated by `kind` (requirements-review | fork | judge | input-gate | approval-gate | agent-decision | clarity-review | brainstorm | pr-review | bug-fishing | human-test | visual-confirmation | follow-ups | interview). A newer deployment may report a variant not in this list."}},"parked":{"type":"boolean"},"runId":{"type":"string"},"status":{"type":"string","description":"One of: running, blocked, paused, done, failed. A newer deployment may report a member not in this list."},"taskId":{"type":"string"},"unanswerable":{"type":"array","items":{"type":"object","properties":{"detail":{"type":"string"},"reason":{"type":"string","description":"One of: human_wait_gate, unclassified_gate, unwired_interview_gate, curation_gate. A newer deployment may report a member not in this list."},"stepIndex":{"type":"number"},"stepKind":{"type":"string"}}}}}},
    invoke: (client, args) => client.decisions.addressBugFishingFindings(str(args, 'runId'), args.body as Parameters<CatFactoryClient['decisions']['addressBugFishingFindings']>[1]),
  },
  {
  ...
  },
  {
    ...
    inputSchema: {"type":"object","properties":{"runId":{"type":"string","description":"Path parameter `runId`."}},"additionalProperties":false,"required":["runId"]},
    outputSchema: {"type":"object","properties":{"artifacts":{"type":"array","items":{"type":"object","properties":{"artifactId":{"type":"string"},"byteSize":{"type":"number"},"contentType":{"type":"string"},"createdAt":{"type":"number"},"hash":{"type":"string"},"kind":{"type":"string","description":"One of: screenshot, reference, asset. A newer deployment may report a member not in this list."},"scope":{"type":"string","description":"One of: run, task. A newer deployment may report a member not in this list."},"view":{"anyOf":[{"type":"string"},{"type":"null"}]}}}}}},
  ...
  },
  {
    ...
    title: 'Acknowledge a Kaizen entry',
    group: 'kaizen',
    operationId: 'acknowledgePublicKaizenEntry',
    readOnly: false,
    description: 'Acknowledge a Kaizen entry\n\nRecord that this entry has been triaged, optionally with a note (a ticket id, why it was dismissed), and take it out of the `acknowledged=false` backlog. Send `{"acknowledged": false}` to undo. A `write` key, not an `admin` one: acknowledging starts nothing and merges nothing. Acknowledging twice is a no-op that returns the row unchanged, so `acknowledgedAt` keeps naming the FIRST triage rather than the last retry. An entry whose grading has not settled yet is refused `409` with `details.reason: "kaizen_entry_not_settled"` (there are no recommendations to have read), and an unknown id is `404` with `details.reason: "kaizen_entry_not_found"`.\n\nCalls `POST /api/v1/kaizen/entries/{entryId}/acknowledge` (operation `acknowledgePublicKaizenEntry`).',
    inputSchema: {"type":"object","properties":{"entryId":{"type":"string","description":"Path parameter `entryId`."},"body":{"type":"object","properties":{"acknowledged":{"type":"boolean"},"note":{"anyOf":[{"type":"string"},{"type":"null"}]}},"description":"The request body."}},"additionalProperties":false,"required":["entryId"]},
    outputSchema: {"type":"object","properties":{"acknowledged":{"type":"boolean"},"acknowledgedAt":{"anyOf":[{"type":"number"},{"type":"null"}]},"acknowledgedBy":{"anyOf":[{"type":"string"},{"type":"null"}]},"acknowledgementNote":{"anyOf":[{"type":"string"},{"type":"null"}]},"agentKind":{"type":"string"},"combo":{"anyOf":[{"type":"object","properties":{"consecutiveHighGrades":{"type":"number"},"verified":{"type":"boolean"},"verifiedAt":{"anyOf":[{"type":"number"},{"type":"null"}]}}},{"type":"null"}]},"comboKey":{"type":"string"},"createdAt":{"type":"number"},"entryId":{"type":"string"},"error":{"anyOf":[{"type":"string"},{"type":"null"}]},"grade":{"anyOf":[{"type":"number"},{"type":"null"}]},"graderModel":{"anyOf":[{"type":"string"},{"type":"null"}]},"model":{"type":"string"},"promptVersion":{"type":"number"},"recommendations":{"type":"array","items":{"type":"string"}},"runId":{"type":"string"},"status":{"type":"string","description":"One of: scheduled, running, complete, failed. A newer deployment may report a member not in this list."},"stepIndex":{"type":"number"},"summary":{"type":"string"},"task":{"anyOf":[{"type":"object","properties":{"serviceId":{"anyOf":[{"type":"string"},{"type":"null"}]},"serviceTitle":{"anyOf":[{"type":"string"},{"type":"null"}]},"status":{"type":"string","description":"One of: planned, ready, in_progress, blocked, pr_ready, done. A newer deployment may report a member not in this list."},"title":{"type":"string"}}},{"type":"null"}]},"taskId":{"type":"string"},"updatedAt":{"type":"number"}}},
    invoke: (client, args) => client.kaizen.acknowledgeEntry(str(args, 'entryId'), args.body as Parameters<CatFactoryClient['kaizen']['acknowledgeEntry']>[1]),
  },
  {
    name: 'kaizen_get_entry',
    title: 'Get one Kaizen entry',
    group: 'kaizen',
    operationId: 'getPublicKaizenEntry',
    readOnly: true,
    description: 'Get one Kaizen entry\n\nThe same entry addressed by its own id, for a caller that stored one (on a ticket it filed, say) and wants the current grade, recommendations and triage state without re-paging the list. Scoped to the calling key’s workspace.\n\nCalls `GET /api/v1/kaizen/entries/{entryId}` (operation `getPublicKaizenEntry`).',
    inputSchema: {"type":"object","properties":{"entryId":{"type":"string","description":"Path parameter `entryId`."}},"additionalProperties":false,"required":["entryId"]},
    outputSchema: {"type":"object","properties":{"acknowledged":{"type":"boolean"},"acknowledgedAt":{"anyOf":[{"type":"number"},{"type":"null"}]},"acknowledgedBy":{"anyOf":[{"type":"string"},{"type":"null"}]},"acknowledgementNote":{"anyOf":[{"type":"string"},{"type":"null"}]},"agentKind":{"type":"string"},"combo":{"anyOf":[{"type":"object","properties":{"consecutiveHighGrades":{"type":"number"},"verified":{"type":"boolean"},"verifiedAt":{"anyOf":[{"type":"number"},{"type":"null"}]}}},{"type":"null"}]},"comboKey":{"type":"string"},"createdAt":{"type":"number"},"entryId":{"type":"string"},"error":{"anyOf":[{"type":"string"},{"type":"null"}]},"grade":{"anyOf":[{"type":"number"},{"type":"null"}]},"graderModel":{"anyOf":[{"type":"string"},{"type":"null"}]},"model":{"type":"string"},"promptVersion":{"type":"number"},"recommendations":{"type":"array","items":{"type":"string"}},"runId":{"type":"string"},"status":{"type":"string","description":"One of: scheduled, running, complete, failed. A newer deployment may report a member not in this list."},"stepIndex":{"type":"number"},"summary":{"type":"string"},"task":{"anyOf":[{"type":"object","properties":{"serviceId":{"anyOf":[{"type":"string"},{"type":"null"}]},"serviceTitle":{"anyOf":[{"type":"string"},{"type":"null"}]},"status":{"type":"string","description":"One of: planned, ready, in_progress, blocked, pr_ready, done. A newer deployment may report a member not in this list."},"title":{"type":"string"}}},{"type":"null"}]},"taskId":{"type":"string"},"updatedAt":{"type":"number"}}},
    invoke: (client, args) => client.kaizen.getEntry(str(args, 'entryId')),
  },
  {
    name: 'kaizen_list_entries',
    title: 'List the workspace\'s Kaizen entries',
    group: 'kaizen',
    operationId: 'listPublicKaizenEntries',
    readOnly: true,
    description: 'List the workspace\'s Kaizen entries\n\nEvery post-run grading the workspace has produced, newest first and keyset-paginated, with no run or task named up front. A Kaizen entry is the platform grading its OWN work: after a run finishes, each completed agent step is judged on how smooth or chaotic the interaction was (1..5) and what would make it better, keyed by the `(agentKind, model, promptVersion)` combo it ran. Each entry carries the context a follow-up needs (the run and step it came from, the agent kind, the resolved model, the prompt version, the board task and its service, and where the combo stands in its verification streak), so acting on one does not mean opening the app first. Filter with `acknowledged=false&settled=true` for the drainable backlog (every entry in it is one the acknowledge route accepts; `acknowledged=false` alone also returns gradings still in flight, which that route refuses with `409`), `settled=true` for everything the grader has finished with whatever it concluded (a `failed` grading names a deployment problem, such as prompt recording being off, and is worth acting on), `status` for one exact grading state, `agentKind` for one role, and `since` for an incremental sweep. A task deleted since the run reports `task: null` rather than a blank title.\n\nCalls `GET /api/v1/kaizen/entries` (operation `listPublicKaizenEntries`).',
    inputSchema: {"type":"object","properties":{"limit":{"type":"integer"},"cursor":{"type":"string"},"acknowledged":{"type":"string","enum":["true","false"]},"settled":{"type":"string","enum":["true","false"]},"status":{"type":"string","enum":["scheduled","running","complete","failed"]},"agentKind":{"type":"string"},"since":{"type":"integer"}},"additionalProperties":false},
    outputSchema: {"type":"object","properties":{"entries":{"type":"array","items":{"type":"object","properties":{"acknowledged":{"type":"boolean"},"acknowledgedAt":{"anyOf":[{"type":"number"},{"type":"null"}]},"acknowledgedBy":{"anyOf":[{"type":"string"},{"type":"null"}]},"acknowledgementNote":{"anyOf":[{"type":"string"},{"type":"null"}]},"agentKind":{"type":"string"},"combo":{"anyOf":[{"type":"object","properties":{"consecutiveHighGrades":{"type":"number"},"verified":{"type":"boolean"},"verifiedAt":{"anyOf":[{"type":"number"},{"type":"null"}]}}},{"type":"null"}]},"comboKey":{"type":"string"},"createdAt":{"type":"number"},"entryId":{"type":"string"},"error":{"anyOf":[{"type":"string"},{"type":"null"}]},"grade":{"anyOf":[{"type":"number"},{"type":"null"}]},"graderModel":{"anyOf":[{"type":"string"},{"type":"null"}]},"model":{"type":"string"},"promptVersion":{"type":"number"},"recommendations":{"type":"array","items":{"type":"string"}},"runId":{"type":"string"},"status":{"type":"string","description":"One of: scheduled, running, complete, failed. A newer deployment may report a member not in this list."},"stepIndex":{"type":"number"},"summary":{"type":"string"},"task":{"anyOf":[{"type":"object","properties":{"serviceId":{"anyOf":[{"type":"string"},{"type":"null"}]},"serviceTitle":{"anyOf":[{"type":"string"},{"type":"null"}]},"status":{"type":"string","description":"One of: planned, ready, in_progress, blocked, pr_ready, done. A newer deployment may report a member not in this list."},"title":{"type":"string"}}},{"type":"null"}]},"taskId":{"type":"string"},"updatedAt":{"type":"number"}}}},"nextCursor":{"anyOf":[{"type":"string"},{"type":"null"}]}}},
    invoke: (client, args) => client.kaizen.listEntries(pick(args, QUERY_KAIZEN_LIST_ENTRIES)),
  },
  {
    name: 'keys_create',
    ...
    outputSchema: {"type":"object","properties":{"key":{"type":"object","properties":{"accountId":{"type":"string"},"actsAsUserId":{"anyOf":[{"type":"string"},{"type":"null"}]},"createdAt":{"type":"number"},"createdByKeyId":{"anyOf":[{"type":"string"},{"type":"null"}]},"createdByUserId":{"anyOf":[{"type":"string"},{"type":"null"}]},"externalIdentity":{"anyOf":[{"type":"string"},{"type":"null"}]},"id":{"type":"string"},"label":{"type":"string"},"lastUsedAt":{"anyOf":[{"type":"number"},{"type":"null"}]},"revokedAt":{"anyOf":[{"type":"number"},{"type":"null"}]},"scope":{"type":"string","description":"One of: read, write, decide, admin. A newer deployment may report a member not in this list."},"workspaceId":{"type":"string"}}},"secret":{"type":"string"}}},
  ...
  },
  {
    ...
    inputSchema: {"type":"object","properties":{},"additionalProperties":false},
    outputSchema: {"type":"object","properties":{"keys":{"type":"array","items":{"type":"object","properties":{"accountId":{"type":"string"},"actsAsUserId":{"anyOf":[{"type":"string"},{"type":"null"}]},"createdAt":{"type":"number"},"createdByKeyId":{"anyOf":[{"type":"string"},{"type":"null"}]},"createdByUserId":{"anyOf":[{"type":"string"},{"type":"null"}]},"externalIdentity":{"anyOf":[{"type":"string"},{"type":"null"}]},"id":{"type":"string"},"label":{"type":"string"},"lastUsedAt":{"anyOf":[{"type":"number"},{"type":"null"}]},"revokedAt":{"anyOf":[{"type":"number"},{"type":"null"}]},"scope":{"type":"string","description":"One of: read, write, decide, admin. A newer deployment may report a member not in this list."},"workspaceId":{"type":"string"}}}}}},
...
export const CAT_FACTORY_TOOL_GROUPS: Readonly<Record<string, string>> = {
  ...
  'models': 'The models a run in this workspace could actually dispatch to, and why an unavailable one is unavailable: unconfigured, or refused by the account model-family policy. Those two need opposite fixes.',
  ...
  'tracker': 'What this workspace does to a task\'s LINKED tracker issue as its pull request progresses: comment when it opens, comment and close the issue when it merges, and post a headless run\'s parked review findings so the reporter can answer where they filed. The write MERGES, so turning one action on leaves the other two as they were. It is the writeback half of the workspace\'s tracker configuration; the filing selection (which tracker a tech-debt ticket is raised on) is not published yet.',
  'risk_policies': 'The risk policies a task can pin, including which is the workspace default: what decides whether a run can land its pull request without a person, and how many attempts its CI fixer, requirement rounds and release watch are given. Broader than merging, which is why it is not called a merge preset.',
  'model_presets': 'The model presets a task can pin, including which is the workspace default: what decides which model each agent step runs on, and so what a run costs. Availability is not repeated here; join `baseModelId` against the models group, which keeps unconfigured and policy-refused apart.',
  ...
  'kaizen': 'The platform\'s own improvement backlog: every post-run grading of an agent step, with the agent kind, model, prompt version and run it came from, what the grader recommended changing, and whether anybody has acted on it yet. Reading takes a `read` key and acknowledging one a `write` key: neither runs anything.',
