...
// The tool table: one entry per `/api/v1` operation the facade exposes, surface version 1.60.0.
//
...
export const CAT_FACTORY_TOOLS: readonly CatFactoryTool[] = [
  ...
  },
  {
    ...
    outputSchema: {"type":"object","properties":{"description":{"type":"string"},"provisioning":{"description":"Discriminated by `type` (kubernetes | custom). A newer deployment may report a variant not in this list."},"serviceId":{"type":"string"},"status":{"type":"string","description":"One of: planned, ready, in_progress, blocked, pr_ready, done. A newer deployment may report a member not in this list."},"title":{"type":"string"},"type":{"type":"string","description":"One of: frontend, service, library, document, api, database, queue, integration, external, environment. A newer deployment may report a member not in this list."}}},
  ...
  },
  {
    ...
    outputSchema: {"type":"object","properties":{"services":{"type":"array","items":{"type":"object","properties":{"description":{"type":"string"},"provisioning":{"description":"Discriminated by `type` (kubernetes | custom). A newer deployment may report a variant not in this list."},"serviceId":{"type":"string"},"status":{"type":"string","description":"One of: planned, ready, in_progress, blocked, pr_ready, done. A newer deployment may report a member not in this list."},"title":{"type":"string"},"type":{"type":"string","description":"One of: frontend, service, library, document, api, database, queue, integration, external, environment. A newer deployment may report a member not in this list."}}}}}},
  ...
  },
  {
    ...
    outputSchema: {"type":"object","properties":{"createdAt":{"type":"number"},"error":{"anyOf":[{"type":"string"},{"type":"null"}]},"failureDetail":{"anyOf":[{"type":"string"},{"type":"null"}]},"failureHint":{"anyOf":[{"type":"string"},{"type":"null"}]},"failureKind":{"anyOf":[{"type":"string","description":"One of: preflight, dispatch, environment, evicted, harness_shutdown, timeout, agent, job_failed, rejected, companion_rejected, stalled, state_unreadable, cancelled, unknown. A newer deployment may report a member not in this list."},{"type":"null"}]},"jobId":{"type":"string"},"progress":{"anyOf":[{"type":"object","properties":{"completed":{"type":"number"},"inProgress":{"type":"number"},"items":{"type":"array","items":{"type":"object","properties":{"label":{"type":"string"},"status":{"type":"string","description":"One of: pending, in_progress, completed. A newer deployment may report a member not in this list."}}}},"total":{"type":"number"}}},{"type":"null"}]},"repoName":{"type":"string"},"repoOwner":{"anyOf":[{"type":"string"},{"type":"null"}]},"repoUrl":{"anyOf":[{"type":"string"},{"type":"null"}]},"serviceId":{"anyOf":[{"type":"string"},{"type":"null"}]},"status":{"type":"string","description":"One of: pending, running, succeeded, failed. A newer deployment may report a member not in this list."},"updatedAt":{"type":"number"}}},
...
export const CAT_FACTORY_TOOL_GROUPS: Readonly<Record<string, string>> = {
  ...
  'environments': 'The cluster this workspace provisions per-run environments onto: probe a candidate connection without saving it, or bind one. The credential is write-only, so a read reports which secret keys are stored and never their values.',
