...
// The tool table: one entry per `/api/v1` operation the facade exposes, surface version 1.16.0.
//
...
const QUERY_DEBUG_LIST_TOOL_CALLS = ['limit', 'cursor', 'jobId', 'order', 'ok'] as const
...
export const CAT_FACTORY_TOOLS: readonly CatFactoryTool[] = [
  ...
  },
  {
    ...
    inputSchema: {"type":"object","properties":{"runId":{"type":"string","description":"Path parameter `runId`."},"limit":{"type":"integer"},"cursor":{"type":"string"},"jobId":{"type":"string"},"order":{"type":"string","enum":["recent","trajectory"]},"ok":{"type":"string","enum":["true","false"]}},"additionalProperties":false,"required":["runId"]},
...
export const CAT_FACTORY_TOOL_GROUPS: Readonly<Record<string, string>> = {
  ...
  'taskTypes': 'What a task can be created AS in this workspace (the built-in kinds plus the operations the deployment registered), and the fields each one accepts.',
