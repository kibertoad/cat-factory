...
// The tool table: one entry per `/api/v1` operation the facade exposes, surface version 1.43.0.
//
...
export const CAT_FACTORY_TOOL_GROUPS: Readonly<Record<string, string>> = {
  ...
  'repos': 'The repositories this workspace can back a service with, and which service each already backs (the discovery half of service creation), plus creating a brand-new one: a bootstrap writes the repository with an agent and reports the board service it materialises.',
