...
import { ENV_VARS, type EnvReadDeps, optionsFromEnv, selectTools } from '../src/config.ts'
...
/**
 * A reader that FAILS, for every case that names no key file.
 *
 * Passing one that throws rather than one that answers makes each of those cases assert something
 * extra for free: that an inline key, a missing variable or a mistyped ceiling never reaches the
 * filesystem on the way to its verdict.
 */
const NO_FILE: EnvReadDeps = {
  readSecretFile: (path) => {
...
describe('optionsFromEnv', () => {
  it('reads the deployment and the key', () => {
    expect(optionsFromEnv(BASE, NO_FILE)).toEqual({
  ...
  it('refuses to start without credentials, naming what is missing', () => {
    expect(() => optionsFromEnv({}, NO_FILE)).toThrow(ENV_VARS.baseUrl)
    expect(() => optionsFromEnv({ [ENV_VARS.baseUrl]: 'https://x.test' }, NO_FILE)).toThrow(
      ENV_VARS.apiKey,
    )
  ...
  describe('the key file', () => {
    ...
    it('refuses both sources at once rather than picking one', () => {
      ...
      expect(() =>
        optionsFromEnv({ ...BASE, [ENV_VARS.apiKeyFile]: '/run/secrets/cat-factory' }, NO_FILE),
  ...
  it('leaves an unset knob ABSENT rather than explicitly undefined', () => {
    ...
    const options = optionsFromEnv(BASE, NO_FILE)
  ...
  it('throws on a mistyped ceiling rather than reverting to the default', () => {
    ...
    expect(() => optionsFromEnv({ ...BASE, [ENV_VARS.maxResultChars]: 'lots' }, NO_FILE)).toThrow(
      ENV_VARS.maxResultChars,
    )
    expect(() => optionsFromEnv({ ...BASE, [ENV_VARS.timeoutMs]: '-1' }, NO_FILE)).toThrow(
      ENV_VARS.timeoutMs,
    )
    expect(() => optionsFromEnv({ ...BASE, [ENV_VARS.maxRetries]: 'many' }, NO_FILE)).toThrow(
      ENV_VARS.maxRetries,
    )
  })
})
...
describe('selectTools', () => {
  ...
  it('keeps only the non-mutating tools in read-only mode', () => {
    ...
    // An operator's switch is the default cause, since it is the only one the stdio server has.
    expect(writeToolsHidden).toBe('configured')
    expect(exposed.length).toBeGreaterThan(0)
    expect(exposed.every((tool) => tool.readOnly)).toBe(true)
  })

  it('carries WHY the writes are hidden, so the instructions can name the right fix', () => {
    // The hosted endpoint derives read-only from the key's own scope, and "the operator switched it
    // off" would send a model to the wrong person: minting a wider key is not editing a host config.
    const byKey = selectTools(CAT_FACTORY_TOOLS, {
      ...options,
      readOnly: true,
      readOnlyReason: 'key-scope',
    })
    expect(byKey.writeToolsHidden).toBe('key-scope')
    // And a reason with no read-only mode is not a state: the tools are all there, so there is
    // nothing to explain.
    expect(
      selectTools(CAT_FACTORY_TOOLS, { ...options, readOnlyReason: 'key-scope' }).writeToolsHidden,
    ).toBeNull()
  })
