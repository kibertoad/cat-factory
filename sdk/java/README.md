...
the metadata Kotlin _reads_ from a Java library can be, and that removes essentially every reason
to maintain a second client:

- **JSpecify `@NullMarked` + `@Nullable`** — the big one. Without it Kotlin's null-checking is
  simply off across the SDK and an NPE surfaces at the first dereference instead of at the call.
