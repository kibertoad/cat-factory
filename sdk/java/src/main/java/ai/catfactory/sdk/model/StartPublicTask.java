// GENERATED by `pnpm gen:sdk` from docs/openapi.json — DO NOT EDIT BY HAND.
// Regenerate after any change to the `/api/v1` contracts; `pnpm check:sdk` fails CI on drift.

package ai.catfactory.sdk.model;

import com.fasterxml.jackson.annotation.JsonIgnoreProperties;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import org.jspecify.annotations.Nullable;

/**
 * The {@code StartPublicTask} wire model.
 * @param pipelineId May be absent entirely. Length 1..120.
 */
@JsonIgnoreProperties(ignoreUnknown = true)
public record StartPublicTask(
    /** May be absent entirely. Length 1..120. */
    @JsonInclude(JsonInclude.Include.NON_NULL) @JsonProperty("pipelineId") @Nullable String pipelineId
) {

    /** A new builder for {@link StartPublicTask}. */
    public static Builder builder() {
        return new Builder();
    }

    /**
     * Fluent builder for {@link StartPublicTask}.
     * Every setter is nullable so a caller supplies only what it means to send. Java has no
     * default arguments and Kotlin cannot synthesise them for a Java constructor, so this is the
     * shape that reads naturally from both languages.
     */
    public static final class Builder {
        private @Nullable String pipelineId;

        /** Set {@code pipelineId}. */
        public Builder pipelineId(@Nullable String pipelineId) {
            this.pipelineId = pipelineId;
            return this;
        }

        /** Build the {@link StartPublicTask}. */
        public StartPublicTask build() {
            return new StartPublicTask(pipelineId);
        }
    }
}
