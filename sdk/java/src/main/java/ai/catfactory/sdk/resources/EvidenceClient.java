...
public final class EvidenceClient {
     ...
     * the throwaway environments it stood up (`state: "live"` is the only one worth opening, and
     * every other row still carries its URL), and the machine checks that ran. The same reduction
     * the app’s outcome card renders, over the same evidence the verification report is built
     * from, so the two cannot state different totals for one run. Nothing here is asserted by a
     * model: every count is derived from recorded verdicts. Prefer the verification report when
     * you need a reviewer’s full bundle; prefer this when you need to say what shipped. Sections
     * state `reported` or `absent` with a machine-readable gap code, and `truncations` names any
     * list the response had to bound.
