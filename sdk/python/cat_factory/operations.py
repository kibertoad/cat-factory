...
class WebhookResource:
    ...
    def set(self, body: PutNotificationWebhook, timeout: float | None = None) -> NotificationWebhook:
        ...
        registered. An omitted field keeps its stored value, so a caller can subscribe to
        run events without re-sending the URL or secret. Supplying `secret` rotates the
        signing secret; omitting it keeps the current one. The endpoint must be `https:` and
        publicly routable unless the deployment widened its allow-list.
