...
class SpecResource:
    ...
    def get_for_run(self, run_id: str, *, timeout: float | None = None) -> PublicRunSpec:
        """Get the specification one run was judged against
        The same in-repo specification the service read serves, read at the branch THIS RUN
        pushed its work to rather than at the repository default. That is the tree a run’s
        verdicts were made against: while its pull request is open, every requirement the
        run itself ADDED is absent from the default branch, so joining `requirements` rows
        from `GET /api/v1/runs/{runId}/report` or `…/outcome` against the service read
        leaves exactly those rows without a criterion. `provenance` names the branch and the
        commit, so a caller can see which tree it got. `anchor` carries one value the
        service read cannot answer, `not_read`: nothing was read, because the run’s spec
        read is gated on a tester having reported so that the tree served is the one the
        verdicts were made against, and `provenance` is null there and only there. The
        refusals are the service read’s: a `503` with `reason: "spec_read_failed"` for a
        repository that could not be read, `"spec_ref_unresolved"` for a branch that would
        not resolve, `"vcs_not_configured"` for a deployment or workspace that wired no
        version control. An outage never reaches a `200`.
        `GET /api/v1/runs/{runId}/spec` (operation `getPublicRunSpec`).
        """
        raw = self._transport.request(
            "GET",
            f"/api/v1/runs/{_quote(run_id)}/spec",
            query=None,
            timeout=timeout,
        )
        return PublicRunSpec.from_dict(raw)
...
class ReposResource:
    ...
    def get_file(self, owner: str, name: str, *, path: str, ref: str | None = None, timeout: float | None = None) -> GetPublicRepoFileResponse:
        """Read one file out of a linked repository
        Read a single file, decoded as UTF-8, from a repository this workspace has LINKED,
        at a branch, tag or commit sha (omit `ref` for the default branch; the response says
        which was used). It exists to answer what a run actually COMMITTED, which nothing
        else on this surface could: the repos reads list rows and reachability, the
        service-spec read serves only the `spec/` tree, and everything else was the agent’s
        own prose, so a caller wanting a real answer had to hold a second source-control
        credential of its own. `path` is a query parameter rather than the rest of the URL
        because a repo-relative path contains slashes and an OpenAPI path segment cannot.
        One file only: there is deliberately no directory listing. A repository this
        workspace has not adopted is a 404 with `details.reason: repo_not_linked`, a path
        the ref does not hold is a 404 with `file_not_found`, and a file past the size this
        read serves is a 422 with `file_too_large` plus its `size` and `limit`: refused
        rather than truncated, because a shortened answer reads exactly like a shorter file.
        `GET /api/v1/repos/{owner}/{name}/contents` (operation `getPublicRepoFile`).
        """
        raw = self._transport.request(
            "GET",
            f"/api/v1/repos/{_quote(owner)}/{_quote(name)}/contents",
            query={"path": path, "ref": ref},
            timeout=timeout,
        )
        return GetPublicRepoFileResponse.from_dict(raw)
...
class UseCasesResource:
    """The deployment's own non-container model operations: what it will generate for you, on
    which models, from which parameters, and running one. Each use case narrows the models
    it may run on and declares the form it accepts, so a wrapper renders a picker from the
    catalog rather than from a hard-coded copy; a model listed as unavailable says whether
    the deployment cannot serve it at all or has yet to configure the credential. Discovery
    takes a `read` key, invoking a `write` one: an invocation spends model tokens and
    returns text, and starts no run.
    """

    def __init__(self, transport: Transport) -> None:
        self._transport = transport

    def get(self, use_case_id: str, *, timeout: float | None = None) -> ListPublicUseCasesResponseUseCase:
        """Get one use case
        Read one registered use case by id: the same projection the catalog returns, for a
        caller that already holds the id and wants the current parameters and model
        availability without paging the catalog.
        `GET /api/v1/use-cases/{useCaseId}` (operation `getPublicUseCase`).
        """
        raw = self._transport.request(
            "GET",
            f"/api/v1/use-cases/{_quote(use_case_id)}",
            query=None,
            timeout=timeout,
        )
        return ListPublicUseCasesResponseUseCase.from_dict(raw)

    def invoke(self, use_case_id: str, body: InvokePublicUseCaseRequest | None = None, *, timeout: float | None = None) -> InvokePublicUseCaseResponse:
        """Run a use case
        Run one use case and answer with the generated text. Synchronous: this is a single
        inline model call with no repository, no container and no run, so there is no job to
        poll. The parameters are validated against the use case’s own descriptors (`422
        use_case_parameters_invalid`, naming every problem at once); a model outside the use
        case’s declared list is refused (`422 use_case_model_not_allowed`) rather than
        substituted, and so is one this deployment cannot serve inline (`503
        use_case_model_unavailable`). An exhausted budget is `429 budget_exhausted`, and a
        model that answers with no usable text is `503 use_case_empty_reply` rather than a
        200 carrying an empty string. A call the vendor did not complete is `503
        use_case_generation_failed`, or `503 use_case_generation_timeout` when it did not
        answer inside the deployment’s per-invocation deadline: separate, because a failure
        is worth surfacing to whoever asked while a timeout is worth retrying with a smaller
        `maxOutputTokens`. `finishReason: "length"` (with `truncated: true`) means the reply
        hit the output budget, so the text is a prefix rather than an answer. Requires a
        `write` key.
        `POST /api/v1/use-cases/{useCaseId}/invocations` (operation `invokePublicUseCase`).
        """
        raw = self._transport.request(
            "POST",
            f"/api/v1/use-cases/{_quote(use_case_id)}/invocations",
            body={} if body is None else _encode(body),
            query=None,
            timeout=timeout,
        )
        return InvokePublicUseCaseResponse.from_dict(raw)

    def list(self, *, timeout: float | None = None) -> ListPublicUseCasesResponse:
        """List the deployment's inline use cases
        List the non-container model operations this deployment has registered: what each
        generates, the models it may run on, the parameters it accepts, and the temperature
        / output bounds an invocation may steer within. Each model carries whether it can be
        served right now, and an unavailable one says which of the two causes it is:
        `provider_unavailable` (nothing here resolves it, so an operator configures the
        provider) or `container_only` (it runs only through a subscription harness inside a
        per-run container, which this surface has none of). An empty list means this
        deployment registered no use cases, not that the surface is missing.
        ...
        return ListPublicUseCasesResponse.from_dict(raw)
...
class EnvironmentsResource:
    ...
    def list_connections(self, *, timeout: float | None = None) -> ListPublicEnvironmentConnectionsResponse:
        """List the environment connections this workspace holds
        Every registered environment handler, with the provision type it serves, the engine
        and backend kind behind it, its endpoint and the secret KEYS it holds, never their
        values. The read half of the connect call, and the half that was missing: a
        deployment that registers its handlers programmatically (the documented path for a
        multi-tenant deployment) had no way for a headless caller to confirm the
        registration landed, so “the backend accepts our credential” and “this workspace has
        a handler for that backend” collapsed into one unanswerable question. It reports
        every engine, including a handler for an environment backend the deployment
        registered in code, so `engine` and `backendKind` are open strings rather than a
        fixed set.
        `GET /api/v1/environments/connections` (operation
...
class ModelsResource:
    ...
    def list(self, *, timeout: float | None = None) -> ListPublicWiredModelsResponse:
        ...
        The workspace’s model catalog with the flags that decide whether an agent step can
        run at all, and which of four unrelated fixes an unrunnable one needs. `available`
        says a run can dispatch to it now. `policyBlocked` says it is configured and refused
        by the account’s model-family policy, so adding another provider key changes
        nothing. `personalSubscription` says it runs on a credential belonging to a PERSON
        (an individual-usage subscription vendor), which a key resolving no user can never
        see. `subscriptionConfigured` then says whether that person actually holds one:
        `true` means the model is wired and only the key’s identity is in the way, `false`
        means the owner is known and holds none, and `null` means there was nobody to ask
        about, so it must not be read as `false`. `userScoped` is SUPERSEDED by
        `personalSubscription` and still answers its original narrower question (whether a
