...
class UseCasesResource:
    ...
    def invoke(self, use_case_id: str, body: InvokePublicUseCaseRequest | None = None, *, timeout: float | None = None) -> InvokePublicUseCaseResponse:
        ...
        use_case_model_unavailable`). An exhausted workspace budget is `429
        budget_exhausted`, and a model that answers with no usable text is `503
        use_case_empty_reply` rather than a 200 carrying an empty string. `finishReason:
        "length"` (with `truncated: true`) means the reply hit the output budget, so the
