...
- **Coverage measurement is nearly absent.** Exactly one package (`kernel`) configures
  ...
  actually cover.
- **`@cat-factory/contracts` coverage is nascent** — the Valibot wire source of truth imported
  by 125 frontend files and every backend. The first logic slice has landed (vitest scaffolding +
  tests for the branchy safety/validation/summary functions — the path/branch-name guards, the
  initiative-preset form suite, the apriori-branch helpers, `accountSettingsSummary`; see
  `initiatives/contracts-test-backfill.md`), but most of the package is still untested.
  `provider-cloudflare` and `provider-s3` are
...
| 3   | Testing         | Enable vitest coverage reporting in the CI test lanes and ratchet-floor the high-value packages (`orchestration`, `server`, `contracts`, `spend`); 🟡 **in progress** — `contracts` now has its first logic slice (was zero); continue the `contracts-test-backfill` initiative.                                                                                                                                                                       | High   | Low–Med |
