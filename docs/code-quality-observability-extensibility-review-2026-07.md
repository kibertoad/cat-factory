...
| 7   | Code quality    | Fix the one N+1 rule violation: `AgentContextBuilder.ts:1050-1052` point-reads `taskRepo.get` inside `Promise.all(map)` — add a chunked-`IN` `listByRefs` port method (D1 ⇄ Drizzle + conformance), and de-hardcode its `'github'`/`'jira'` literals.                                                                                                                                                                                             | Medium | Low     |
