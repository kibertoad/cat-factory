...
- **Staleness debits** (the flip side of carrying this much documentation; one from the
  ...
  - `backend/docs/custom-agents.md`'s registration sample is **fixed** (it now shows the
    ...
    generic body path).
  - `AGENTS.md`'s i18n claim ("most components still hold inline strings") still materially
...
| 16  | Docs            | Staleness sweep: fix `custom-agents.md`'s Status section (the sample itself is fixed now), the `AGENTS.md` i18n claim, and `refactoring-candidates.md`'s line counts (drifted further; `jobBody.ts` 440 → actual 950); convert the modular-vue and registry-DI trackers to ADRs (52 open trackers and counting).                                                                                                                                                                                                                                           | Low    | Low     |
