...
**Excluded as expensive proprietary frontier** (per the brief): the
...
direct-key "go premium" overrides, out of scope here. OpenAI's **open-weight**
`gpt-oss` models _are_ on Workers AI and _are_ in scope: they are not ChatGPT.

**Excluded as superseded / old models** (brief: "only use latest"). Dropped from
...
| **architect** (design)                   | No          | Long-form solution design, components, trade-offs                                   | High reasoning, structured long-form prose                                            |
| **reviewer** (code review)               | No          | Assess a diff for correctness/security/risk, ordered findings                       | High reasoning over code, calibrated judgment, structured output                      |
...
| **tester**                               | No          | Define high-value test cases by impact                                              | Mid reasoning                                                                         |
| **acceptance**                           | No          | Requirements → Given/When/Then scenarios                                            | Mid, structured output                                                                |
| **business-reviewer**                    | No          | Compare changes vs documented rules, flag drift                                     | Mid reasoning + recall                                                                |
| **blueprints**                           | ✅         | Emit canonical service→module→feature **JSON** tree                                 | Mid + **reliable structured/JSON output**                                             |
| **requirement-reviewer**                 | No (inline) | Generate review items (gaps/risks/questions) from requirements                      | Mid reasoning, structured output                                                      |
| **document-planner**                     | No (inline) | Extract board tree from imported docs (degrades to heading parser)                  | Low-mid, structured output                                                            |
| **researcher / documenter / integrator** | No          | One-shot research / docs / integration notes                                        | Low-mid                                                                               |
| **fragment-selector**                    | No (inline) | Pick relevant prompt fragments (tiny classification)                                | **Lowest**; cheapest model wins                                                      |
...
benchmarks three tasks, `requirement-review`, `code-review`, and `implementation`
...
- **implementation (coder):** `glm-5.2` (incumbent agentic default) vs
  `kimi-k2.7-code` vs `gpt-oss-120b` vs `qwen3-30b-a3b` as a value long-shot.
