...
|---|-----------|------|--------|--------|
| 1 | Split `ExecutionService` | Backend engine | Very high | Very high |
| 2 | Manifest-driven agent-kind registry | Backend engine | High | Medium |
| 3 | Generic row mappers | Backend persistence | Medium | Low |
| 4 | Module registry for the orchestration container | Backend DI | High | High |
| 5 | Shared container builder (Node ⇄ Cloudflare) | Cross-runtime | High | High |
| 6 | Shared OpenAI-compatible provider registry | Cross-runtime | Medium | Low |
| 7 | Split the `ui.ts` store | Frontend | High | Medium |
| 8 | Store pattern factories | Frontend | Medium | Low |

Recommended sequencing: start with the low-effort wins (#3, #6, #8) to build momentum and
de-risk, then the registry refactors (#2, #7), then the structural ones (#1, #4, #5).

---

## 1. Split `ExecutionService` into step handlers + a completion-resolver registry
...
isolation, and removes the implicit-ordering hazard. Highest effort of the set — best done
incrementally, one step kind at a time, behind the cross-runtime conformance suite.

## 2. Manifest-driven agent-kind registry (finish the strangler work)
...
two switches in three files.

## 3. Generic row mappers
...
schema-drift bugs, and shrinks the per-field change surface from 3–4 edits to 1.

## 4. Module registry for the orchestration container
...
testable, and removes the implicit-ordering footgun. Pairs naturally with #5.

## 5. Shared container builder (Node ⇄ Cloudflare)
...
two. Compose with #4 so the shared builder consumes the module registry.

**Why high-impact.** Eliminates the single largest parity-maintenance hazard in the repo
and makes "what does a container wire?" answerable in one place.

> Note: the related "shared base repositories" idea (D1 ⇄ Drizzle dedup) was considered but
> **not** selected for this round; it can be revisited after #5 lands.

## 6. Shared OpenAI-compatible provider registry
...
entry that both runtimes pick up automatically.

## 7. Split the `ui.ts` store
...
shrinks the surface a feature must understand, and enables selective hydration.

## 8. Store pattern factories
...
and gives one place to fix/optimize list mutation and integration error handling.
