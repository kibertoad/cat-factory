...
| **follow-up** | **cross-workspace mount of a personal repo** (redaction resolves `linked_via` from the viewing workspace's projection only, so a personal repo mounted from a sibling workspace is not classified/redacted) | todo   |
