...
- **Every default build pipeline is service-shaped.** All of them hard-code
  `… → deployer → tester-api → …` (since the catalog collapse: the `pl_build`/`pl_simple`/`pl_full`
  ladder, kernel `seed.ts` `buildDeliveryPipelines`), and pipeline selection knows nothing of frame
  types;
...
1. **Dedicated library pipelines auto-picked by default** (a `pl_library_full` /
   ...
   service-shaped behaviour. This is the "copy of the machinery" shape AGENTS.md warns
   against.
...
| 7   | **Pipeline selection ignores frame type.** `defaultPipelineIdForTaskType` handles only `document`; the positional-first fallback can hand a library task any service pipeline. With #1-#3 landed the damage is contained (steps adapt), but selection/pickers still offer misleading choices (e.g. `pl_visual`, or any build rung's `deployer`/`tester-api` tail on a library frame).                                                                                   | Extend the picker/run-start predicate (the `frameAllowsVisualPipeline` pattern, generalised over the frame profile) so pipelines whose steps cannot apply on a library frame are filtered/refused with an actionable message; optionally add a frame-type-aware default (task on a `library` frame with no pin ⇒ the leanest applicable build pipeline).                                                                                      | todo   |     |
...
| 10  | **Doc/prompt drift around the old local-infra model.** AGENTS.md still describes `localTestInfraSupported`/"limited mode" steering in `tester-infra.logic.ts` (removed); `testerEnvironmentSection` narrates `docker-compose` as "stood up on localhost" while `testerInfraSpec` ships it as `ephemeral`: the lock-step comment notwithstanding, the wording and the wire spec tell different stories. | Fix AGENTS.md's tester-infra paragraph while touching this area; reconcile the compose wording in `testerEnvironmentSection` with the actual spec (and with the new library-local mode from #2) so prompt and harness agree again.                                                                                                                                                                                                                                                                                                                                 | todo        |       |
| 11  | **No coverage.** Nothing asserts library-frame behaviour: no conformance case for "library frame ⇒ deployer skips, tester gets suite posture + local compose spec", no detection fixture shaped like shared-ts-libs, no e2e spec importing a library repo.                                                                                                                                             | Conformance: drive a build pipeline on a `library` frame via `FakeAgentExecutor` on both runtimes; assert the deployer no-op reason, the tester `infra` spec (`local`+`composePath`), and prompt posture. Detection unit fixtures: a shared-ts-libs-shaped monorepo. E2e: import-as-library flow once the UI slices land. **Landed (#1110): the cross-runtime conformance case (deployer no-op + tester-to-completion, compose path declared) + `frameProfile`/`testerEnvironmentSection`/`testerInfraSpec` unit coverage. Detection fixtures + e2e remain todo.** | in-progress | #1110 |
...
- **Anything that grows the harness job wire shape or `standUpInfra` behaviour is an
  ...
  (see AGENTS.md's image-tag rules).
...
- **No DinD-availability guard yet for infra-needing libraries (deferred, #4/D2).** After
  #1110 the tester-infra start gate passes through for library frames (`liveTestable:false`)
  and `testerInfraSpec` emits `{ environment: 'local', composePath }`, so a library that
  declares a compose path on a runtime WITHOUT Docker-in-Docker (Apple `container`, native
  local mode, Cloudflare Containers) will fail at RUN time when `standUpInfra` tries
  `docker compose up`, with no early refusal. D2 recommends option (a): refuse at run start
  with an actionable `TESTER_INFRA_MESSAGES`-shaped message, resolving `localDind` the way the
  local facade already surfaces it as `localTestInfraSupported`. Wire this alongside gap #4
  (coder/ci-fixer infra threading) so the guard covers every infra-consuming kind at once.
- **`testPosture` is the tester's run-mode source of truth.** The suite-vs-exploratory choice
  is keyed off `frameProfile(type).testPosture === 'suite'` in BOTH `testerEnvironmentSection`
  (agents) and `testerInfraSpec` (server), not off `deployable`/`liveTestable` (which drive
  the deployer no-op and the infra start gate respectively). Keep the tester pair on
  `testPosture` so the lock-step can't drift for a future frame kind whose posture and
  deployability diverge.
