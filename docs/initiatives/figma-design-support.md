...
- [x] **Live URL on the outcome card.** The run's environments are a section of the outcome
      ...
      captured views on the card and on `GET /api/v1/runs/:runId/outcome` (surface 1.36.0, outcome
      ...
      **The section reports a STATE, not a URL**, which is the decision the plan above did not
      ...
      Three producers know something about the same environment and they are read in strict
      precedence: the DISPOSE record wins (it is written after the run's polls stop refreshing the
      projection), the step projection beats the deployer's terminal row (it follows the
      environment while the run watches it), and the deploy row is the floor. A reclaim that
      FAILED deliberately changes nothing here: the environment is still standing and its URL still
      works, and that it should not be is the verification report's teardown proof, not this card's
      question.
      ...
      **The in-flight row is reported and LABELLED** (`origin: 'projected'`), because a card that
      ...
      The rules this shares with the verification report (which step carries the per-frame
      outcomes, which recorded states mean an environment is gone, whether the deployer declared
      retention) moved into contracts' `run-evidence.ts` beside the tester rules, and the parity
      test asserts the two documents name the same environments. A clock is deliberately NOT one
      of the inputs: a lapsed TTL is reported as the instant it lapses, so the SPA composing the
      summary live and the endpoint composing it server-side cannot disagree about one run.
