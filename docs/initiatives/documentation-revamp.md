...
- [x] 7. SDKs and MCP. Two website pages (SDKs, MCP server); `sdk/README.md` reduced to the
      ...
      gotcha below.
- [~] 8. Custom agents, gates, providers, frontend extensions. Website owns authoring and the repo
  docs open with the pointer, but only the POINTER landed: `custom-agents.md`'s "The three
  stages" still restates the site's "The mental model: three stages" almost line for line, and
  the same holds for its seams and worked example. Finish under item 15.
...
- [ ] 15. **Finish the reductions the pointers only announced.** A pointer at the top of a doc is
      ...
      doc in the "mixed" row gained a pointer over unchanged prose, which is the
      two-parallel-full-accounts state rule 1 exists to end. One doc per PR; the per-section plan is
      below.
...
## Slice 15: the per-doc reduction plan

The trap that shapes all three: **the site splits a feature from its MANIFEST.** A manifest schema
lives on `extend/manifests.html`, not on the feature's own page, so a repo doc reducing its manifest
