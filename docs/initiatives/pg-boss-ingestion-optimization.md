...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ | ------------------------------------------------------ | ----- |
| V1  | Verification spike: pg-boss v12 `insert()` option surface — confirm `JobInsert` carries everything `sendOptions()` uses (`singletonKey`, `retryLimit`, `retryDelay`, `retryBackoff`, `expireInSeconds`, `heartbeatSeconds`) and that batch insert respects the `exclusive` policy's ON-CONFLICT dedup | —                        | ✅ done                                                | (this PR) |
| B2  | Batch the spend-paused re-drive loop (`pgBossRunner.ts:337-358`) via `insert([...])`                                                                                                                                                                                                                  | V1                       | ✅ done                                                | (this PR) |
| B1  | Opportunistically batch the stale-run re-drive sends (`pgBossRunner.ts:240-317`) where kinds/options allow                                                                                                                                                                                            | V1, B2                   | ✅ done                                                | (this PR) |
