...
**Status:** analysis complete, no slices started · **Owner:** core · **Started:** 2026-07-11
...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ | ------------------------------------------------------ | --- |
| V1  | Verification spike: pg-boss v12 `insert()` option surface — confirm `JobInsert` carries everything `sendOptions()` uses (`singletonKey`, `retryLimit`, `retryDelay`, `retryBackoff`, `expireInSeconds`, `heartbeatSeconds`) and that batch insert respects the `exclusive` policy's ON-CONFLICT dedup | —                        | ⬜ todo                                                |     |
| B2  | Batch the spend-paused re-drive loop (`pgBossRunner.ts:337-358`) via `insert([...])`                                                                                                                                                                                                                  | V1                       | ⬜ todo                                                |     |
| B1  | Opportunistically batch the stale-run re-drive sends (`pgBossRunner.ts:240-317`) where kinds/options allow                                                                                                                                                                                            | V1, B2                   | ⬜ todo (adopt-later)                                  |     |
...
- **Preserve the `exclusive` dedup semantics under batch insert.** The `singletonKey`
  ...
  failure) before converting any loop.
