...
1. **Phase A** ✅ (#1129) — gaps 1 + 2 (+ the prompt side of 5): the `spike` kind, `pl_spike`, the
   type default. Lands the happy path — and since Phase B's terminal-state fix already shipped
   (see footnote ¹), the spike run finishes cleanly at `done` end-to-end.
2. **Phase B** ✅ (already landed) — gap 3 (+ the copy part of the `pipeline_complete` fix): the
   engine's no-PR completion path shipped ahead of this initiative (`RunStateMachine.finalizeBlock`).
   Independent of Phase A; already unblocks every PR-less pipeline.
