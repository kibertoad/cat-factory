...
The good news is that **all the machinery exists**: read-only `container-explore` kinds with
...
- **Dedicated result window (if warranted)**: the result-view seam; `resultView` id in
  `catalog.ts` → the first-party `resultViews` slot in `app/modular/result-views.ts` (read by
  `StepResultViewHost.vue` via `resolveComponentRegistry`).
...
- **This PR: P0 (gaps 1–3) + P1 (gaps 4–7) + the backend half of gap 11.** Landed the
  built-in `spike` kind (`agents/kinds/spike.ts`: read-only `container-explore`, structured
  findings + prose `summary`, `generic-structured` view) and its findings post-op (renders
  `docs/research/<slug>.md` (honouring `taskTypeFields.targetPath`) and commits it on the base
  branch via `RepoFiles`, no PR); the seeded `pl_spike` (`requirements-review`(off) → `spike`) +
  the `spike → pl_spike` type default; the engine's **no-PR completion path**
  (`RunStateMachine.finalizeBlock`: a task run that opened no PRs finishes `done` instead of
  stalling at `pr_ready`); the timebox + research-criteria prompt fold (`spikeContextSection`);
  the spike create fields (`successCriteria` added to `taskTypeFieldsSchema`, reusing
  `researchQuestion`/`optionsToCompare`) + the `AddTaskModal.vue` section + i18n across all 10
  locales; and the cross-runtime conformance assertions (drive `pl_spike` → `done`, findings on
  `step.custom`, post-op commit, AND the repo-less docs-only settle). **Decisions taken:**
  **D1** = in-repo `docs/research/…` commit (recommended); **D2** = structured findings with a
  mandatory prose `summary` (recommended); **D3** = auto-`done` (no human-ack gate: the no-PR
  path in gap 3), leaving a `findings_ready` acknowledgement variant to a later slice if wanted.
- **Gap 6 resolution:** a repo-less spike (GitHub unwired, or a docs-only spike) settles on
  `step.custom`: the post-op is skipped, not fatal (asserted in conformance); a block under an
