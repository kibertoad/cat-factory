...
window is per-isolate, so it is "a speed bump, not an authoritative limiter", and it names
a durable, cross-runtime limiter (D1/Postgres-backed, exercised by the conformance suite)
as the proper follow-up. Concretely:
...
2. **Implementations**: a small `auth_attempts` table, D1 ⇄ Drizzle (+ migrations both
   ...
   stores; the port comment requests exactly this.
...
4. **Retention**: attempts are junk minutes later, so hook `deleteOlderThan` into the
