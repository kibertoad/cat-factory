...
- **`act` is `admin`-scoped, the side-effect is shared, and the retry tail stays headless-safe.**
  ...
  inbox and the public route — do NOT re-inline it. A public `act` that would RETRY a run
  (`ci_failed` / `test_failed`) reuses `personalGateForRun` to refuse an individual-usage-model run
  up front (→ 409 `individual_model_unsupported`), exactly like the retry endpoint; the merge tails
  need no personal credential and run headless (no `usr_*` initiator → installation token).
