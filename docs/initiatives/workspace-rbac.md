...
**Status:** design complete (no slices landed) · **Owner:** core · **Started:** 2026-07-16
...
- **Migrations**: D1 `backend/runtimes/cloudflare/migrations/0051_workspace_rbac.sql`
  (next free number) ⇄ Drizzle `backend/runtimes/node/src/db/schema.ts` +
  `pnpm db:generate`. New table ⇒ FKs are born clean, no heal-then-constrain needed.
