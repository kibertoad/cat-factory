...
| Gates                  | kernel + `@cat-factory/gates` | `kernel/domain/gate-registry.ts`               | ✅ done (`GateRegistry` + `defaultGateRegistry()`; `registerBuiltinGates(registry)` — the module-load side-effect + `registerBuiltinGates()` band-aid are gone)  |         |
...
| Step resolvers         | kernel                        | `domain/step-resolver-registry.ts`             | ✅ done (`StepResolverRegistry` + `defaultStepResolverRegistry()`; read via `CoreDependencies.stepResolverRegistry`)                                             |         |
...
## Gate + step-resolver slice — carried gotchas

Landed together (they are the two registries the engine's `RunDispatcher` reads):
...
- **Providers stay module-global (deliberately).** A built-in gate's `wired()` /
  ...
  and leaves the gate-provider tests unchanged — provider tokens are their own (paired) next
  slice.
- **`validateRegistrations` now takes the gate registry** (like it already took the agent-kind
