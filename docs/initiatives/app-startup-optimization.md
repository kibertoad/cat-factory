...
socket opened before the init fetch committed, but the first shape is cleaner (one code
...
background regardless, see `backend/runtimes/node/src/redisProbe.ts`), yet when `REDIS_URL`
...
`getMe` leg, which requires loosening `AuthGate`'s render gating) is deferred; it composes with item
...
documented invariant; rejecting it is an acceptable outcome, but record the rejection here.
...
`BoardCanvas.vue`), which is defensible since the board IS the app's first view, and
probably `markdown-it` via eagerly-imported inspector/step-prose paths. No bundle-size
...
accept it (Workers cold starts are already rare per-isolate events). Do not restructure the
