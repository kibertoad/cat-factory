...
- **No upstream code change was needed to install** — with the pin raised, cat-factory consumes the already-published `@modular-vue/*` directly (no local link). The co-evolution half is therefore docs + one peer widen (below), not a blocking release.
- **Open re-adopt (the one co-evolution tail).** cat-factory is on `vue-router@5.x` (Nuxt owns it) while `@modular-vue/{core,runtime,nuxt}` peer-require `vue-router@^4.5.0` — a non-fatal peer warning (our zero-route Option-B path never touches the router at runtime). Fix filed upstream in [modular-react#87](https://github.com/kibertoad/modular-react/pull/87) (widen the peer to `^4.5.0 || ^5.0.0`). Re-adopt = bump to the released widened version once published; that closes the warning. Slice 0's cat-factory code is complete and green regardless.
