...
| 2   | Working mode: context threading + `workBranch` override + probe-only ensure (executor + `RunDispatcher` ×3 sites) + merger deletion guard + tests                                                       | `kernel/src/ports/agent-executor.ts`, `orchestration/…/AgentContextBuilder.ts`, `server/src/agents/ContainerAgentExecutor.ts`, `orchestration/…/RunDispatcher.ts`, `server/src/github/GitHubPullRequestMerger.ts` | ✅ done     | TBD  |
