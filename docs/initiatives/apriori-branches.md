...
**Status:** in progress (slices 1–3 landed: data model + working mode + reference mode) · **Owner:** core · **Started:** 2026-07-10
...
| 2   | Working mode: context threading + `workBranch` override + probe-only ensure (executor + `RunDispatcher` ×3 sites) + merger deletion guard + tests                                                       | `kernel/src/ports/agent-executor.ts`, `orchestration/…/AgentContextBuilder.ts`, `server/src/agents/ContainerAgentExecutor.ts`, `orchestration/…/RunDispatcher.ts`, `server/src/github/GitHubPullRequestMerger.ts` | ✅ done     | #999      |
| 3   | Reference mode: harness `referenceBranches` fetch (⇒ image bump) + `jobBody` prompt section + per-kind gating + dispatch probe                                                                          | `executor-harness/src/{job,git,coding-agent,agent}.ts`, `server/src/agents/{jobBody,ContainerAgentExecutor}.ts`                                                                                                   | ✅ done     | (this PR) |
