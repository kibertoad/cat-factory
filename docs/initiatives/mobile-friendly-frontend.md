...
| ID  | Sev | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Where                                                                          | Status |
| --- | --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ | ------ |
| C1  | P2  | Sub-12px typography sweep. The shared scale half is DONE (#2248): the pixel literals are gone, `text-2xs` (0.6875rem) and `text-3xs` (0.625rem) are declared in `app/assets/css/type.css`, and `check-frontend-type-scale.mjs` keeps them the only spelling. The LEGIBILITY half is still open and is what remains of this row: those steps render at the same 11px and 10px they always did. Because they are now two `@theme` values rather than ~1650 literals, bumping the band on a compact viewport is a media-query override in one file, not a migration. Decide the compact-viewport floor (≥12px was the original ask) and apply it there. | `frontend/app/app/assets/css/type.css`                                         | todo   |
...
| D3  | P2  | Frame resize edge strips only reach 16px under `pointer-coarse` (`w-4`/`h-4`) while the corner correctly reaches 44px; widen edges to ~`w-8`/`h-8` under `pointer-coarse`. (Was frame **and module**; a module no longer renders as a resizable box since tasks moved into swimlanes, so only the frame's grips remain.)            | `board/nodes/ResizeGrips.vue`                                         | todo   |
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| E1  | A mobile _reading model_ for the board (list/drill-down instead of spatial pinch-zoom). Task cards are fixed 210px (`utils/laneGeometry.ts`) and pipeline detail only appears at zoom ≥1.8 (`useSemanticZoom.ts:13-19`), so reading a large board on a phone means constant pinch-panning. The task swimlanes narrow this: a frame is now a fixed-size, internally-scrolling column set rather than an unbounded canvas, so the remaining gap is the three-column width at phone viewports, not the whole spatial model. | Big surface, separate initiative if phone usage becomes primary. The A–D items make the board _operable_ on touch; the inspector + notifications already provide a serviceable non-spatial reading path. Decide after A–D land and real usage is observed. |
