...
// This branch wrote 1.12.0, which main then published for `configUnreadable`, and 1.13.0 went to
// the run-debugging surface, so the number moves twice in one merge. Re-checked against
// `origin/main`; a sibling branch in flight also claims 1.14.0, so whichever of the two lands
// second re-checks this line again rather than trusting its clean merge.
