// The public API's `info.version` and the log of every step it has taken.
//
// Split out of `generate-openapi.mjs` when the log pushed that file past its size budget, along
// the seam the two already had: the emitter renders whatever contracts hold, while this is the
// running record of what changed on a STABLE surface and why each step was additive. The log grows
// once per public-API change and the emitter does not, so keeping them together meant the emitter
// carried the cost of every version that had nothing to do with it.
//
// Read by `generate-openapi.mjs` alone. `check-openapi.mjs` grades the rendered document, so it
// sees this through the same call.
...
//
// 1.38.0: one new value, `consensus_panel`, on the same `reason`. It says the step was diverted to a
// multi-model consensus panel, whose participants are inline model calls with no agent CLI, so there
// was nowhere to wire an MCP server. Additive, on the same terms as 1.37.0: a consumer built against
// it sees a value it cannot classify rather than a wrong one.
//
// A member of its own rather than `harness_unsupported` because no harness is involved. The kind's
// standard surface may serve the server perfectly and the same step with consensus off would have
// got it, so a consumer acting on the harness reason would go widening a `harnesses` list that was
// never the constraint. It is also the first member a consumer can see on a step that reached no
// container at all, which is why the record now arrives from the inline path too.
